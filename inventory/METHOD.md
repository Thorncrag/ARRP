...
Every audit tier should include a current-status check for mootness and material reframing, scaled to the tier's estimate. If recent decisions, legal opinions, enacted laws, regulations, agency actions, court orders, factual developments, repeals, superseding enactments, or other current events materially alter the proposal, document the finding on the issue page, update the audit row, and adjust the score. If reliable current sources show the standalone proposal has become completely moot, set the score to `0` while that condition remains true.
...
Audit inventory rows should not hide material audit results. When an audit updates [`audits.csv`](audits.csv), also update the relevant issue page with a detailed public-facing audit record that includes the latest audit tier, date, proposal-quality score, corrections made, unresolved findings, material caveats, score limitations, source-review summary, and next audit need. Also update [`../AUDIT_DASHBOARD.md`](../AUDIT_DASHBOARD.md) so project-wide audit status remains current for meta-analysis. Prefer too much reader-facing audit detail over too little: the issue page should explain what was checked, what was verified, what was fixed, what remains unresolved, and why the score changed or did not change. Any audit information a human reader would reasonably need should appear on the issue page, while the CSV row should serve as the compact tracking record and the dashboard should serve as the compact cross-issue view.

Issue-page audit history should be append-only. Add each new audit as a new dated entry under the issue page's Audit Record, newest first by default, and retain prior entries even if the log grows long. Do not erase, replace, or compress earlier audit entries except to correct clerical errors, broken links, stale line references, or clearly identified inaccuracies. The public compiled document may omit or trim audit logs separately; the source issue page remains the complete technical audit record.
