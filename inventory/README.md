...
Both source catalogs use `Monitoring` to identify changing records. A monitored source must also carry a concise `Monitoring Rationale` explaining the development being watched and a `Monitoring Group` that joins records concerning the same matter. Those source-level fields do not replace `needs: monitoring` on a proposal or formal-candidate GitHub issue, which triggers review of the whole issue and an active search for additional developments.
