...
- [`sources.csv`](sources.csv) is the citation catalog for external sources actually used in ARRP prose, whether linked externally or retained in [`../sources/`](../sources/).
- [`sources-pending.csv`](sources-pending.csv) holds stable-ID sources awaiting verification, monitoring, adjudication, or reader-facing placement. It is not a citation catalog; move a row into `sources.csv` when the source is actually cited.
