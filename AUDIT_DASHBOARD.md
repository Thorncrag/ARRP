...
Horizon Scan audits identify possible new, emerging, or newly salient concerns within the project's goals. They are project-wide discovery reports, not direct implementation passes. Each scan should apply the Issue-Admission Test, cross-check existing issues and legislation for duplication or overlap, and recommend whether to create a new issue, merge into an existing issue, expand or amend an existing issue, cross-reference, source-develop, or decline the concern.
