...
**Finding and correction:** The Mueller and Watergate examples remain distinct manifestations of executive-controlled investigative limits and evidence-preservation needs. *Trump v. United States* supplies controlling legal context rather than a third historical manifestation, so its unique immunity proposition and citation were consolidated into Annotation. The *Phang v. Blanche* cross-reference remains correctly bounded: present statutory-compliance litigation does not establish an offense, extraordinary executive conflict, or an appointment trigger.

**Internal remedy-fit result:** DOJ-007's constitutional amendment, preferred JUD-011-compatible implementation, and independent alternative remain aligned. A later authenticated finding could matter only through the proposal's independent misconduct, conflict, application, and appointment criteria.
...
**Next audit need:** External validation / T4 follow-up.

### 2026-07-15 — Epstein Files conditional-investigation source update

**Classification:** Source-development and cross-reference update; not a completed Change Audit or T0-T4 run.

**Change:** Added *Phang v. Blanche* to explain the boundary between the Epstein-files statutory-compliance litigation and DOJ-007's extraordinary independent-investigation mechanism.

**Boundary:** The current preliminary injunction and show-cause posture do not establish a possible offense, obstruction, concealment, evidence destruction, retaliation, or extraordinary executive conflict. Later authenticated evidence of comparable serious misconduct may support an application only through an authorized applicant and only if every independent appointment criterion is satisfied.

**Audit control:** The score remains 78/100 and Runs remains unchanged. Because the new current-case cross-reference expands the source and remedy-fit basis without a contemporaneous Change Audit, `change_audit_needed` is set to `true`. The next targeted Change Audit must refresh the docket and verify that DOJ-007 remains conditional rather than automatic.

### 2026-07-14 — JUD-005 Conditional-Escalation Change Audit

**Audit type:** Targeted Change Audit and Internal Remedy-Fit review; not a T0–T4 run and no score recalculation.

**Scope:** Reviewed both DOJ-007 enabling Acts, the issue page, appointment standard, authorized-application and court-referral routes, JUD-001 and JUD-005 overlap, firewalls, and current dual-path budget architecture.

**Change:** Both enabling Acts now state expressly that a JUD-001 or JUD-005 noncompliance notice or public-register entry does not establish criminality, probable cause, extraordinary executive conflict, or entitlement to appointment. Such material may support only an application by an already authorized applicant, and every independent misconduct, conflict, proportionality, and appointment criterion remains applicable.

**Internal remedy-fit finding:** DOJ-007 remains an extraordinary investigation proposal rather than an enforcement or automatic-referral mechanism. JUD-005 routes authenticated information; JUD-001 enforces the judicial order; DOJ-007 activates only where additional facts indicate possible serious misconduct and disabling executive conflict. The amendment, preferred JUD-011 implementation path, standalone alternative, separate panels, decisional firewalls, and budgets remain unchanged.

**Score effect:** No change; 78/100 remains current. The new language is an abuse-resistance and overlap clarification and does not resolve external constitutional review, grand-jury disclosure, cost, adoption, or stakeholder gaps.
...
**Records updated:** Updated DOJ-007 issue page, created `legislation/DOJ-007-amendment.md`, updated `legislation/DOJ-007.md`, updated `FRAMEWORK.md`, `FRAMEWORK.md`, `PRINT_ASSEMBLY.md`, `inventory/contents.csv`, `inventory/audits.csv`, and the Audit Dashboard row.
