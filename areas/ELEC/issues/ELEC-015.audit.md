...
**Totals correction:** Replaced candidate-count and ballots-cast ranges with event-level totals where the harvested data permits. Candidate participation now reports total candidates across the extracted contests for each event. Voter turnout now uses a single unique-ballot event figure where available, or a labeled party-primary ballot proxy using statewide Democratic and Republican ballot totals where full event turnout has not yet been reduced.
...
**Alternative-vote metric pivot:** Replaced candidate count as the headline Maine participation metric with an alternative vote-expression measure. The revised measure tracks the share of nonblank contest votes cast for candidates or officially tabulated vote categories outside the Democratic and Republican nominees, including printed independent/minor-party candidates and official write-in/other columns where Maine reports them. The source-development table now explains that RCV contests use first-choice totals only and that presidential-year rows use the statewide presidential contest for same-office comparison. The graph was replotted to show alternative vote expression above turnout, with the x-axis extended to 2006 while marking pre-2010 official Maine extraction as pending because Maine's current online archive provides 2010-forward files and directs users to the Division of Elections for earlier result requests.
