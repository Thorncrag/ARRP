...
This folder contains ARRP's separately deployed public interaction service, available at [gab.durablerepublic.org](https://gab.durablerepublic.org/). It is deliberately separate from the read-only [`ARRP Project Console`](../framework/project/interfaces/project-console/project-console.html) console. Both application-like surfaces use the [project-operated interface visual standard](../framework/FRAMEWORK.md#project-operated-interface-visual-standard); that standard does not replace the main GitHub Pages or print themes.
...
[`../framework/project/workflows/public-input-review.md`](../framework/project/workflows/public-input-review.md) governs interactive Codex review and expressly authorized scheduled Elim review of public-intake comments. No GitHub Action or separately billed external-model service performs semantic assessment. Review otherwise remains report-only, but Elim may post one validated, self-identifying, informative reply when an existing ARRP proposal or prior recorded disposition answers the submission, and it may automatically create or update a fully sourced preliminary-candidate record for a distinct unowned institutional question. Those exceptions do not authorize promotion to `HOR-###`, admission, rejection, moderation, deletion, private contributor contact, publication, or other project-record changes. Every authorized action receives an Intake Action Ledger record.
