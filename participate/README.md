...
- `api/submit` — validates the request, checks the bot token and a narrow server-side privacy preflight, resolves a deterministic proposal/area/general route, finds or creates the corresponding GitHub Discussion, adds the submission as a comment through a narrowly scoped GitHub App, and, when an address is authorized, sends it only to the private ARRP follow-up mailbox.
