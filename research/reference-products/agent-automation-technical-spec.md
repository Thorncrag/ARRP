...
2. quarantines only tightly allowlisted Finder-style duplicate Git metadata and numbered project-tree copies that map exactly to files tracked at `HEAD`, retains their hashes and bounded history, removes a detected copy from any newly staged boundary, verifies canonical state, and fails closed on unknown or repeatedly regenerated artifacts; then fetches `origin/main`, requires the canonical checkout to be on `main` with local `HEAD` exactly equal that revision, preserves ordinary uncommitted paths through an exact-base checked pull request and squash merge before deferring to the next poll, resumes an interrupted prepared workspace commit only after exact parent, identity, message, path, diff, check, and merge readback, verifies that the local dispatcher runtime byte-matches the synchronized revision, creates or refreshes the isolated Elim workspace, and immediately persists the verified clean checkout head before any later launch decision;
...
13. creates a bounded branch, stages only the declared paths, commits under the coordinator identity, and either leaves a human-review pull request open or requires every reported check to finish without failure before an exact-base squash merge and readback;
...
| Installed host poll | macOS launchd runs `/opt/homebrew/bin/python3 /Users/benjaminsmith/Automation Workspaces/ARRP/scripts/run_chain_dispatcher.py --launch-codex` every 600 seconds from the non-File-Provider automation checkout. | Host lease, local control and history, downloaded or fetched manifest, verified inputs, and launch state. | It may consume an already completed authorized chain. It does not manufacture one. |
...
| 10 | Host preflight and manifest retrieval - run-chain dispatcher | The dispatcher fetches `origin/main` and requires the canonical checkout to be on `main` at the exact fetched revision. If ordinary uncommitted paths remain, it rejects conflicts or staged diff-check failure, commits the complete workspace on a bounded host branch, opens an exact-head and exact-base pull request, requires every reported check to finish without failure, squash-merges only while `origin/main` remains at the pinned base, reads back the exact merge, returns local `main` to that boundary, and exits until the next poll. If publication stopped after that commit, only the recognized clean host branch may resume after its parent, identity, message, paths, diff, PR, checks, and merge are revalidated. Otherwise it verifies the byte-for-byte automation runtime and either uses `gh run watch` and `gh run download` for a newly dispatched run or fetches the current data-branch manifest. It never rebases, resets, force-pushes, bypasses required checks, or resolves divergent history. | The user workspace and chain baseline both equal reviewed `origin/main`, the Chain ID is neither consumed nor terminally failed, and every fetched projection matches its recorded hash. |
...
| 19 | Trusted-host Git closeout - run-chain dispatcher | The host validates the result, handoff, exact Run Log field names, material provenance, selected-unit binding, and exact diff; then creates `codex/elim-CHAIN_ID`, stages only declared files, and commits. A human-review result remains on an open unmerged pull request. Any other accountably closed result uses an exact-head and exact-base pull request, bounded all-check wait, no-base-movement gate, squash merge, and exact merge readback. A clean prepared host commit may be revalidated and resumed after interrupted publication. | The exact branch commit, pull request, checks, and merged or intentionally open boundary are read back. The model-authored result is preserved separately from the host-enriched result. |
...
The automation workspace is isolated from the user's interactive checkout, but dispatch does not begin while its canonical host checkout contains unreconciled work. That checkout resides at `/Users/benjaminsmith/Automation Workspaces/ARRP`, outside the File Provider-managed Documents domain. The dispatcher inspects the checkout and every ancestor for `com.apple.file-provider-domain-id` before Git preflight and fails closed if the boundary is managed or cannot be verified. It then validates the allowlisted `Thorncrag/ARRP` origin, fetches `origin/main`, and requires the canonical checkout to be on `main` at that exact revision. If ordinary uncommitted paths remain, it rejects unresolved conflicts, creates a bounded branch, stages the complete workspace, requires a clean staged diff check, commits under the coordinator identity, opens an exact-head and exact-base pull request, waits within the reviewed bound for every reported check to finish without failure, confirms that `origin/main` has not moved, squash-merges without bypass, reads back the exact merge, returns local `main` to that boundary, then exits until the next host poll reloads the synchronized runtime. If that publication is interrupted after commit creation, a later poll resumes only a branch matching the bounded host-workspace identity after proving the commit has one exact parent encoded by the branch, the reviewed host author and committer identity and message, a nonempty safe changed-path set, clean diff, exact pull-request head and base, passing reported checks, and exact merge readback. An already merged PR is not accepted from merge state alone; its checks are read back again. It must not rebase, reset, force-push, bypass required checks, or resolve divergent history. A linked Git worktree is deliberately not used: its `.git` file would point into the canonical checkout's metadata outside the controlled full checkout. Instead, the dispatcher maintains one fixed ignored full checkout at `.tmp/run-coordinator/elim-checkout`, with its own real `.git` directory available to the trusted host. It advances that isolated checkout only from a previously recorded successful checkout head, verifies the selected manifest revision, and launches Elim there. A dirty or unrecorded divergent isolated checkout is preserved and fails closed rather than reset.
...
The workspace-write model does not stage, branch, commit, push, or create a pull request. Its result must report `commit: null`, an empty synchronization list, every changed working-tree path exactly once, and one current Run Log table using every exact nonblank contract field name. The host rejects an undeclared or missing path or a field alias; validates run, provenance, intake, handoff, selected-unit, and Review Epoch evidence as applicable; stages only the declared set; and creates the commit. Before any network write, it requires a clean tree, the pinned baseline as the commit's sole parent, and a committed path set identical to the verified pre-commit declaration. A `human_review` result is pushed to an open, unmerged bounded pull request. Another accountably closed result is pushed only to its bounded branch, opened as an exact-head and exact-base pull request, held for every reported check, and squash-merged only if the base remains pinned; the exact merge must read back from `origin/main`. A clean prepared host commit is revalidated before any publication retry. Remote movement, authentication failure, failed or timed-out checks, or a readback mismatch preserves the checkout and fails closed. The original model result and host-enriched result remain distinguishable.
...
- non-File-Provider storage, dirty workspace, prepared-workspace resumption, branch divergence, isolated-checkout, lock contention, stale-lock, and interruption tests;
