...
- **Overview** is the compact daily operations briefing, not a second Progress dashboard. It reports the effective run-chain and host-closeout posture, the latest recorded Elim result and any issue-level improvements, confirmed human actions, all Integrity findings, monitored issues, open pull requests, public intake, recent project actions and accepted Horizon decisions, project-data freshness, official OpenAI GPT/Codex/API service health, the latest private host-attested Codex usage posture, and an Elim per-run usage trend. A complete queue directory indexes active, empty, and unavailable operational queues without inventing a zero when a live check fails. A failed, stale, or blocking worker produces a prominent alert linked directly to that worker's card. Every portal routes to its complete owning view rather than creating another authority. **Request fresh overview** sends a bounded run request only through the approved localhost Run Coordinator service; it neither edits project data nor promises that Elim will launch.
