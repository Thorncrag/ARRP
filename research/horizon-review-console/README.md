...
- **Overview** is the compact daily operations briefing, not a second Progress dashboard. It shows one latest-activity portlet for every registered agent and bot, with the latest actual execution result as the headline and current-chain scheduling posture as secondary context. A successful result therefore remains **Succeeded** when that automation was merely not due in the present chain. Recent activity is limited to automated actions for now. The page also reports confirmed human actions, all Integrity findings, monitored issues, repository reviews, public intake, project-data freshness, official OpenAI GPT/Codex/API service health, the latest private host-attested Codex usage posture, and an accessible line graph of Elim consumption by run. The complete queue directory presents every queue as its own portlet; a zero-count queue says **Empty**, while a failed live check says **Unavailable** rather than inventing a zero. Alerts and portlets route to their complete owning view. Repository proposals route to the dedicated Action Items listing, which matches the live pull-request head to a checked-in Elim or interactive recommendation; GitHub is retained as secondary evidence. **Request fresh overview** sends a bounded run request only through the approved localhost Run Coordinator service; it neither edits project data nor promises that Elim will launch.

- **Progress** is the sole human-facing Review Ready planning view. Summary metrics are followed immediately by the compact six-stage development-level board; the largely duplicative closest-to-ready list has been removed. Schedule, trajectory, compact area coverage, workflow holds, and issue-level monitoring remain available as supporting sections. Compact area coverage, workflow-hold groups, and monitored-issue headings remain visible for at-a-glance scanning; each list-bearing group or issue is separately collapsible and initially collapsed. The board keeps all six maturity stages in one desktop row and uses identifier-only cards with score, live-page, GitHub-issue, and workflow cues. It corresponds to the GitHub Project **Development Level** board, not the separate nine-column **Status** board. GitHub Project `Development level` remains the maturity authority and `Status` remains the workflow authority. The projected Status vocabulary is `Research`, `Development`, `Human decision needed`, `Audit needed`, `Audit in progress`, `External review`, `Publication approval`, `Deferred`, and `Blocked`; these describe repeatable workflow or hold categories rather than development stages or whether someone is actively working at that moment. The checked-in bundle preserves an offline snapshot; when network access is available, the view refreshes from the data-only `project-console-data` feed.

- **Action Items**, immediately after Progress, is the central inbox for proposal and candidate records carrying `Human decision needed`, preliminary-candidate review, unresolved source routing, human-owned Integrity findings, failed or blocking automation, and open pull requests awaiting disposition. Routine issue monitoring is excluded unless it produces a decision or exception requiring attention. The page-level count is the sum of the complete entries listed in its queue cards, which appear two per row on ordinary desktop widths and one per row on narrow screens. The cards remain visible rather than collapsing and open the complete owning view without creating a second workflow authority. Locally staged print-level changes remain exclusively in Publication.
...
- **Integrity** preserves the complete inspectable exception inventory, not just matters requiring human attention. It combines deterministic Integrity findings, actionable source-check exceptions, candidate-dossier completeness gaps, Project tracking and lifecycle warnings, publication metadata exceptions, and operational-readiness conditions. The four current run-status portals live here beside the findings; complete historical runs remain under Logs. Unrecognized or missing Status values remain visible for correction instead of being silently translated. Problems are grouped first by the human, agent, bot, or observation state accountable for the next step, and every owner group shows its complete count. Each problem retains its category, stable reference, affected records, reporting source, responsibility class, state, severity, last-check date, and direct record link. Filters select an accountable owner, severity, or state; only the human subset enters Action Items.
- **Agents & Bots** opens to **Administration**, an overview of every registered automation role plus current run-chain health, queue counts and ownership, Elim's launch decision, Review Epoch, usage posture, per-stage latest-success and current-schedule details, and recovery state. Its localhost-only coordinator controls use plain-language actions, selectable published work items and unresolved alerts, safe defaults, and confirmations for consequential requests. They may request a run or comprehensive review, create or clear only user-owned queue overrides, or preserve a resolution record for an automation alert; unavailable-service and file-preview states remain read-only. The separate **Agents & Bots** detail view is generated from `framework/agents/` and parses every second-level section of each authoritative runbook. Each role has one full-width card containing identity, status, trigger, environment, runtime and configuration, model policy where applicable, latest actual result, current-chain disposition, recovery posture, source links, and filtered log access. The linked runbook remains controlling.
- **Logs** provides searchable, sortable, groupable, unpaginated views labeled **Horizon**, **Elim**, **Bots**, **Sources**, **Integrity**, and **Changes**. Elim retains its dedicated run log; **Bots** presents the canonical bot and historical autonomous-action log; **Sources** presents material watcher-change history; and **Integrity** contains bounded run history while current run-status portals and unresolved findings remain in Integrity. The latest matching record is presented in a dedicated summary, while earlier records appear as compact, full-width lines that expand on demand; canonical Markdown and generated feeds remain authoritative.
