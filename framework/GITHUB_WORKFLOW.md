...
- `Deferred / Parked`, `Blocked`, and `Completed within scope` — use only when their stated routing or terminal conditions actually apply. A canonical issue mapped to `Deferred / Parked` must carry a nonblank `workflow_hold_reason`; the Project status alone is not a sufficient explanation.
