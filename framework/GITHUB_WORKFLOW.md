...
The deterministic monitoring prototype is currently paused while its source-coverage strategy is reconsidered: the GitHub Actions workflow is manually disabled and `.github/monitoring-pilot.json` is also disabled as a second guard. If deliberately re-enabled, it is configured to run daily at 12:13 a.m. in the `America/New_York` timezone and may also be dispatched manually. It discovers every open GitHub issue carrying `needs: monitoring`, matches permanent `SRC-####` associations in both source catalogs, checks each source supported by an available deterministic adapter, and identifies records for which no automated check is configured. It may add one rolling `## Automated monitoring check` comment to each monitor and update that same comment on later runs. The comment stores only source fingerprints and concise structured observations needed to compare runs. Every completed scheduled or manual run also posts a new concise outcome report to the dedicated [Monitor-bot run reports issue](https://github.com/Thorncrag/ARRP/issues/317), mentions the project owner, and links to the Actions run; this per-run notice is an operational notification, not a canonical change log, while detailed comparison state remains on the individual monitor. The present signal-only bot cannot modify repository or source records, and the workflow verifies that boundary. A run that makes no repository change does not append to the Agent Audit Log. Any later source-writing capability requires separate authorization and a dedicated branch and pull request. The source-changing PR must itemize every added, updated, moved, or removed stable source ID; identify the reason, destination, citation or proposition supported, and originating Actions run; append the corresponding autonomous-change entry to the [Agent Audit Log](logs/AGENT_AUDIT_LOG.md); and rely on the PR diff and merged Git history as the authoritative record of prior and new values, timestamp, and authorship. A run notice may link that PR but does not replace it. A temporary `needs: monitor review` label means the configured source fields changed or the automated check failed repeatedly; it is a triage signal, not a finding that the monitor's legal or factual reassessment trigger occurred. A substantive reviewer decides significance and performs any required repository, source, monitoring, or Change Audit updates. After that review, removing `needs: monitor review` acknowledges the current structured observation as the next automated baseline. The automation never removes `needs: monitoring`, closes an issue, modifies Project fields, or edits substantive repository content.
