...
8. when the audit changes an eligible proposal's Project `Status`, `Score`, or goal eligibility, manually dispatch the Project Console progress-data workflow after Project readback and push, wait for completion, and verify `project-console-data/progress.json` reflects the new portfolio state; and
...
7. when the audit changes an eligible proposal's Project `Status`, `Score`, or goal eligibility, a successful Project Console progress-data refresh and readback, or an explicit recorded blocker identifying the failed workflow or stale generated state.
