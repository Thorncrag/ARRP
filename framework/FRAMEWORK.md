...
3. **Proposed candidate.** An approved preliminary becomes a formal `HOR-###` candidate and GitHub issue. Candidate adjudication verifies the premise, checks existing coverage, applies the issue-admission and political-failure tests, tests ripeness and reversed-party neutrality, and recommends admission, integration, deferral, source retention, retirement, or rejection. The approved disposition remains a human decision.
4. **Admitted issue.** Independent admission converts the existing Horizon issue into a stable area-specific issue while preserving its Horizon provenance. The admitted issue receives the canonical records required by the [Horizon Candidate Adjudication Workflow](#horizon-candidate-adjudication-workflow), but it may remain `Pending development` or `In development` while one or more foundational decisions are unresolved.
5. **Defined proposal.** The admitted issue becomes a Defined Proposal once the human has approved the institutional failure, at least one manifestation, the remedy, and the remedy vehicle. `Defined Proposal` is a substantive lifecycle stage, not a score band or separate GitHub Project status. It establishes the foundation for broad delegated development; it does not imply that the issue page, vehicle, evidence, or implementation analysis is complete. For an unscored proposal, record the approval durably in the canonical issue metadata before a scheduled run treats the proposal as Defined.
...
7. **Developed proposal and audit readiness.** A Defined Proposal becomes Developed when the initial issue-and-vehicle package performs the required proposal functions described under the [Proposal Quality Score](#proposal-quality-score). When that package is sufficiently complete for a productive T-audit, an unscored proposal moves to `Audit needed`. Readiness means that an audit can test the proposal meaningfully; it does not mean that the proposal is expected to pass or advance.
...
9. **Review Ready.** A score of 75 or higher establishes Review Ready or a higher threshold status under the current rubric. Review Ready is the project's portfolio-development target: the proposal is strong enough for knowledgeable external critique, not permanently complete.
...
11. **Maintenance.** Published or review-ready work remains subject to monitoring, new evidence, legal developments, implementation experience, and material revision. A material revision triggers the ordinary Change Audit rules without automatically erasing the proposal's prior score-based status.
...
Scores, prior tier completion, word count, citation count, and Project status are useful evidence but are not rigid gates. Higher tiers remain cumulative under the Audit Depth rules. A proposal may be ready to be tested even when it is unlikely to advance, while a superficially complete or highly scored proposal may require development or a Change Audit before another T-audit would be useful.
...
When an issue page displays a Proposal Quality Score, the threshold status should appear in parentheses immediately after the score, for example: `82 / 100 (Review Ready)`. GitHub Project status fields should use the same threshold-status labels for formula-based proposal scores when practical; active fixed zero-status items may use routing labels such as `Deferred / Parked` or `Pending development` when those labels are more informative. Reserve `Completed within scope` for work whose defined obligation is actually complete, not as a generic merger, integration, retirement, archival, or publication label.
