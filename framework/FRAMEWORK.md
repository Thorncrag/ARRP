...
5. **Defined proposal.** The admitted issue becomes a Defined Proposal once the human has approved the institutional failure, at least one manifestation, the remedy, and the remedy vehicle. Its Project `Development level` is `Defined proposal`. This is a substantive maturity stage, not a score band or workflow status. It establishes the foundation for broad delegated development; it does not imply that the issue page, vehicle, evidence, or implementation analysis is complete. For an unscored proposal, record the approval durably in the canonical issue metadata before a scheduled run treats the proposal as Defined.
6. **Development.** Within the approved foundation, agents may research, draft, expand, reorganize, test, and improve the issue page, proposal vehicle, supporting evidence, implementation design, budget treatment, and connected project records. An incomplete initial package remains `In development`.
7. **Developed proposal and audit readiness.** A Defined Proposal becomes Developed when the initial issue-and-vehicle package performs the required proposal functions described under the [Proposal Quality Score](#proposal-quality-score). Its `Development level` becomes `Developed proposal`. When that package is sufficiently complete for a productive T-audit, its workflow `Status` becomes `Audit needed`. Readiness means that an audit can test the proposal meaningfully; it does not mean that the proposal is expected to pass or advance.
...
An admitted issue becomes a **Defined Proposal**, and substantive autonomous development begins, after the human has approved four foundations:
...
For an unscored proposal, preserve that approval in canonical issue front matter as `foundation_status: approved`, `foundation_approved_date: YYYY-MM-DD`, and an optional concise `foundation_approval_note` identifying the human decision or project record. `foundation_status: pending` means that one or more foundations still require human decision. Absence of the field is not permission for a scheduled run to infer approval. A proposal already scored through a completed T-audit is treated as a legacy-established foundation unless its current record expressly identifies a foundational question; add the metadata when it is next materially revised.
...
### Recurring Autonomous Development Order

A scheduled daily LLM development run should perform its substantive work in this order:

1. **Resolve actionable Change Audits first.** Review every proposal whose GitHub Project `Change audit needed` field is `Yes` or `Pending review`. Complete each review that can be resolved within the human-approved foundation, synchronize the affected issue, vehicle, audit sidecar, sources, metadata, and Project fields, and clear the marker only after readback confirms the resolution. A Change Audit does not increment `Runs` unless a distinct T0-T4 audit is separately completed and recorded.
2. **Escalate blocked or reserved Change Audit decisions.** If a flagged review depends on a human-reserved decision or cannot safely be resolved, document the uncertainty and proposed options, leave the marker in the appropriate unresolved state, add the matter to the human-review queue, and continue. One blocked Change Audit must not prevent work on other eligible issues.
3. **Perform ready T-audits.** After all actionable Change Audits have been resolved or routed for human review, perform the next useful T-audit for proposals classified `Ready for T-audit`, using Project priority, release-blocker posture, age, and likely contribution to the Review Ready goal to order otherwise comparable work.
4. **Develop Defined Proposals.** Use remaining capacity to advance Defined Proposals classified `Development needed first` toward audit readiness, beginning with their recorded next steps and highest-value unresolved deficiencies.
5. **Report and synchronize.** Report every completed audit, lifecycle advancement, skipped or blocked decision, and exact next action; update all governing repository and GitHub surfaces required by the ordinary workflow rules.

The recurring run may research and recommend candidate dispositions, but it must not make a human-reserved admission, merger, split, retirement, rejection, foundational, or publication decision. Those items should appear as concise human action items rather than silently consuming the development queue.
...
1. an admitted proposal whose four-part foundation remains unresolved uses `Development level: Admitted / undeveloped`; its workflow `Status` is `In development` while actively being worked or `Pending development` while queued;
2. a human-approved foundation uses `Development level: Defined proposal` until the initial issue-and-vehicle package is substantively complete;
...
GitHub Issues and the Project Monitoring view are the issue-level monitoring-workflow authority. The ARRP Project Console's **Sources** tab contains a read-only **Watchers and bots** view with **Court Cases**, **Presidential Directives**, and **Issues being monitored** subviews; do not maintain a duplicative overview screen. It groups monitored matters by accountable ARRP owner and, where useful, by a named case family, executive-action episode, or other factual matter. Every view must distinguish: (1) why the matter is monitored and what development would trigger review; (2) whether a deterministic watcher or an LLM-assisted pass covers it; and (3) its latest known posture. Routine issue monitoring is intended for automated LLM-assisted review and is not itself a human Action Item merely because `needs: monitoring` remains present. Only a detected development, exception, or resulting decision that requires human attention enters Action Items. The console does not create, close, edit, or adjudicate an issue. The **Sources** and **Pending sources** subviews expose each source's `Monitoring` value so a changing docket or similar record remains identifiable even when the owning issue is not currently in an issue-wide monitoring pass. Ordinary badges state the complete queue size. A visually distinct update badge states only a genuinely new or changed record awaiting review, using the directive registry status or an unresolved deterministic-watcher pull request; do not use the update color merely to restate a queue total.
...
Use this mathematical formulation only for developed proposals. A **Defined Proposal** becomes **Developed** when the project has created a basic proposal framework for the issue: a distinct issue page or equivalent proposal page that identifies the institutional anomaly, manifestation, damage, underlying weakness, proposal survey or source-development basis, least-complex adequate remedy, repair/prevention approach, and proposed legislative or non-legislative vehicle where appropriate. Candidate inventory entries, area-page issue bullets, source-development notes, and Defined Proposals whose initial package is still incomplete are not developed proposals and receive no proposal-quality score until that framework exists.
...
When an issue page displays a Proposal Quality Score, the threshold status should appear in parentheses immediately after the score, for example: `82 / 100 (Review Ready)`. The GitHub Project `Development level` should reflect the corresponding substantive threshold, while `Status` records the next workflow action or hold. Active fixed-zero items retain their defensible development level and may use workflow states such as `Deferred / Parked` or `Pending development`. Reserve `Completed within scope` for work whose defined obligation is actually complete, not as a generic merger, integration, retirement, archival, or publication label.
