...
3. **Proposed candidate.** An approved preliminary becomes a formal `HOR-###` candidate and GitHub issue. Its Project `Development level` is `Candidate`; its workflow `Status` identifies the actual next action or hold. Use `Development` while candidate research or dossier work is next, `Human decision needed` once a specific human-reserved disposition is ready, and `Deferred` or `Blocked` only under the defined hold tests below. Candidate adjudication verifies the premise, checks existing coverage, applies the issue-admission and political-failure tests, tests ripeness and reversed-party neutrality, and recommends admission, integration, deferral, source retention, retirement, or rejection. The approved disposition remains a human decision.
4. **Admitted issue.** Independent admission converts the existing Horizon issue into a stable area-specific issue while preserving its Horizon provenance. Its Project `Development level` is `Admitted / undeveloped` until a stronger maturity threshold is established. The admitted issue receives the canonical records required by the [Horizon Candidate Adjudication Workflow](#horizon-candidate-adjudication-workflow). Its workflow `Status` is `Development` while permitted foundation or proposal work is next, or `Human decision needed` when a specific missing foundation requires a human-reserved choice.
...
| --- | --- |
| `Development` | Ordinary research, drafting, source development, structural work, or revision is the next workflow. It covers both work waiting to begin and work already underway; it does not function as an activity indicator. |
...
Do not change `Status` merely because a work session starts or stops. If ordinary development is the next workflow, use `Status: Development` throughout. Do not reduce `Development level` merely because revision begins. If the revision is material, preserve the established maturity and use the separate `Change audit needed` field and the Status identifying the actual next action or hold until the review is resolved.
...
2. a foundation established directly by the human or through a recorded Elim or interactive-agent four-criterion sufficiency determination uses `Development level: In development` and ordinarily `Status: Development` until the initial issue-and-vehicle package is substantively complete;
