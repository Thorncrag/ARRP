...
5. Area `issue_count` front matter counts active records only, including active candidates, paused records, developing issues, and developed proposals. It excludes records finally merged, integrated, retired, rejected, or moved elsewhere. Update the count when any such transition occurs.
6. Substantive maturity should use the GitHub Project `Development level` field, while the separate `Status` field records the current workflow action, hold, or terminal posture. Do not use audit work such as `Audit needed` as a substitute for the proposal's development level.
...
3. **Proposed candidate.** An approved preliminary becomes a formal `HOR-###` candidate and GitHub issue. Its Project `Development level` is `Candidate`; its workflow `Status` identifies whether it awaits decision, is deferred, or is blocked. Candidate adjudication verifies the premise, checks existing coverage, applies the issue-admission and political-failure tests, tests ripeness and reversed-party neutrality, and recommends admission, integration, deferral, source retention, retirement, or rejection. The approved disposition remains a human decision.
4. **Admitted issue.** Independent admission converts the existing Horizon issue into a stable area-specific issue while preserving its Horizon provenance. Its Project `Development level` is `Admitted / undeveloped` until a stronger maturity threshold is established. The admitted issue receives the canonical records required by the [Horizon Candidate Adjudication Workflow](#horizon-candidate-adjudication-workflow), but its workflow `Status` may remain `Pending development` or move to `In development` while one or more foundational decisions are unresolved.
...
8. **T-audit development.** T0 through T4 audits progressively test triage, framework compliance, substantive development, external-circulation readiness, and publication readiness. Only completed and separately recorded T-audits increment `Runs`; scoring and lifecycle changes follow the [Audit Rules and Proposal Quality Scoring](#audit-rules-and-proposal-quality-scoring).
9. **Review Ready.** A score of 75 or higher establishes a `Development level` of `Review ready` or a higher threshold under the current rubric. The workflow `Status` separately identifies the next action, ordinarily external review unless a Change Audit or other prerequisite intervenes. Review Ready is the project's portfolio-development target: the proposal is strong enough for knowledgeable external critique, not permanently complete.
10. **External review and publication development.** Qualified external review, resulting revisions, and any remaining legal, drafting, fiscal, implementation, adoption, or stakeholder work may advance the proposal through Advanced Review Ready, Proposal Ready, Publication Ready, or Fully Validated. Human authorization remains required for final external circulation or publication.
11. **Maintenance.** Published or review-ready work remains subject to monitoring, new evidence, legal developments, implementation experience, and material revision. A material revision triggers the ordinary Change Audit rules without automatically erasing the proposal's prior score-based development level.

Merged, integrated, rejected, retired, moot, deferred, or monitored records follow the disposition and preservation rules applicable to their stage. `Completed within scope` is reserved for an obligation that is genuinely complete; it is not a synonym for publication, merger, retirement, or inactivity.
...
For recurring autonomous triage, classify each plausible proposal as `Ready for T-audit`, `Development needed first`, `Change Audit needed first`, `Waiting on external development`, or `Human decision required`. State the recommended tier and a concise reason. If the scheduled work expressly authorizes tier selection, choose the lowest useful next tier rather than invoking the ordinary clarification rule for an unspecified audit request.
...
If substantive work begins while the Project `Status` is `Pending development`, update it to `In development` and read the row back. Do not reduce `Development level` merely because revision begins. If the revision is material, preserve the established maturity and use the separate `Change audit needed` field and an appropriate workflow `Status` until the review is resolved.

At closeout:

1. an admitted proposal whose four-part foundation remains unresolved uses `Development level: Admitted / undeveloped`; its workflow `Status` is `Awaiting decision` when the missing foundation requires human choice, `In development` while authorized foundation work is actively underway, or `Pending development` while that work is queued;
2. a foundation established directly by the human or through a recorded Elim or interactive-agent four-criterion sufficiency determination uses `Development level: In development` until the initial issue-and-vehicle package is substantively complete;
...
4. a completed T-audit score of 1–49 or 50–74 retains `Development level: Developed proposal`; `Status` identifies whether development or another audit is the next action, while the issue page retains the exact `Early/Partial Draft`, `Developed Draft`, or `Substantially Developed Draft` threshold label;
5. a score of 75 or higher uses `Development level: Review ready` and ordinarily `Status: External review needed`, unless an unresolved Change Audit or another recorded prerequisite makes a different workflow state more accurate; and
6. `Development level: Release candidate` is used only after the separate release-candidate requirements are affirmatively satisfied; `Status` then records the remaining publication action.
...
A **project-wide monitoring pass** is a non-scoring review of every open proposal or formal candidate carrying `needs: monitoring`. It is distinct from a T-audit and does not alter proposal scores, lifecycle status, or audit-run counts merely because an external matter remains open.
...
GitHub Issues and the Project Monitoring view are the issue-level monitoring-workflow authority. The ARRP Project Console separates issue workflow from source inspection: **Progress** contains the read-only **Issues being monitored** group, while **Sources** contains Court Cases, Presidential Directives, and Source Checks. The Progress group organizes monitored matters by accountable ARRP owner and, where useful, by a named case family, executive-action episode, or other factual matter. Every view must distinguish: (1) why the matter is monitored and what development would trigger review; (2) whether a deterministic watcher or an LLM-assisted pass covers it; and (3) its latest known posture. Routine issue monitoring is intended for automated LLM-assisted review and is not itself a human Action Item merely because `needs: monitoring` remains present. Only a detected development, exception, or resulting decision that requires human attention enters Action Items. The console does not create, close, edit, or adjudicate an issue. The **Sources** and **Pending sources** subviews expose each source's `Monitoring` value so a changing docket or similar record remains identifiable even when the owning issue is not currently in an issue-wide monitoring pass. Ordinary badges state the complete queue size. A visually distinct update badge states only a genuinely new or changed record awaiting review, using the directive registry status or an unresolved deterministic-watcher pull request; do not use the update color merely to restate a queue total.
...
Reader-facing intake terminology uses **preliminary candidate** for a synthesized question that has no `HOR-###` identifier or GitHub issue, and **proposed candidate** for a formal `HOR-###` record awaiting admission, merger, monitoring, retirement, or another disposition. Preserve **Horizon** in internal technical names, identifiers, logs, metadata, GitHub labels, and workflow instructions. The ARRP Project Console has seven primary tabs in this order: **Progress**; **Action Items**, the central linked inbox for human decisions, unresolved routing, and detected-update review; **Candidates**, containing candidate and preliminary-candidate subviews; **Sources**, containing cited sources, pending sources, watchers, and bot views; **Integrity**, presenting the latest automated consistency findings and retained run history; **Logs**, containing searchable, sortable, groupable projections of the canonical Horizon Scan, Agent Audit, Source Monitor, and historical Change Audit logs; and **Publication**, containing page publication dispositions, edition analysis, and a document builder. Those views are non-authoritative projections: lifecycle and candidate authority remains in GitHub, bibliographic authority remains in the two source catalogs, each project log remains authoritative in its canonical Markdown file, presidential-directive identity and review history remains in the directive registry, and print disposition and assembly authority remain in page front matter and [`print-assembly.json`](print-assembly.json). The Publication assignment view must make included, explicitly excluded, unclassified, and conflicting outcomes independently visible. Routine issue monitoring remains in Sources for automated LLM-assisted review and does not count as a human Action Item unless that review produces an exception or decision requiring attention. Locally staged publication changes remain in Publication and likewise do not count as Action Items. The Action Items view must route each count to its complete owning view and must not duplicate narrative workflow records. The Logs view is read-only, retains complete entries without pagination, and links to each authoritative log; it must not become a separately maintained ledger. Publication controls may stage and export instruction lists for Codex, but do not persist drafts or edit canonical files.
...
3. Conduct a systematic internal-consistency review of governing project materials before applying the change to any individual proposal. This review should be deep enough to catch the kinds of drift that arise from repeated structural edits rather than only obvious formatting errors. The following checks are a required floor, not an exhaustive ceiling; the auditor should follow any additional inconsistency, ambiguity, broken reference, stale convention, or implementation defect discovered during the review. At minimum, check this document, [`PRINT_ASSEMBLY.md`](PRINT_ASSEMBLY.md), [`GITHUB_WORKFLOW.md`](GITHUB_WORKFLOW.md), retained source-inventory headers, issue-page templates as reflected in developed pages, audit sidecar conventions, and Horizon Scan rules for:
   ...
   - metadata/front-matter drift, including `issue_id`, `area_id`, `status`, `remedy_type`, `legislative_proposal`, `constitutional_proposal`, `audit_*` fields, publication disposition (`print_levels` or documented exclusion), and audit-history paths;
...
6. **Proposed Legislation**, **Proposed Constitutional Amendment**, or **Proposed Enabling Legislation** uses Markdown bullet links or an approved `Pending development` placeholder;
...
Before starting substantive audit work, confirm whether the issue has a concrete proposal vehicle. Check the issue page's **Proposed Legislation**, **Proposed Constitutional Amendment**, or **Proposed Enabling Legislation** section, any linked legislation files, and the GitHub Project status fields. If the issue has no linked draft vehicle and instead uses `Pending development`, another no-draft placeholder, or no proposed-vehicle section, notify the user before proceeding and ask for confirmation. The notice should state plainly that no proposed legislation or equivalent proposal text exists yet, that any audit will be limited to source development, issue admission, remedy selection, or fixed-zero/candidate review unless drafting is added to the scope, and that a formula-based Proposal Quality Score should not be assigned until a concrete draft exists. Do not treat the `Pending development` placeholder as a broken link.
...
When an issue page displays a Proposal Quality Score, the threshold status should appear in parentheses immediately after the score, for example: `82 / 100 (Review Ready)`. The GitHub Project `Development level` should reflect the corresponding substantive threshold, while `Status` records the next workflow action or hold. Active fixed-zero items retain their defensible development level and may use workflow states such as `Deferred / Parked` or `Pending development`. Every canonical issue mapped to `Deferred / Parked` must include a nonblank machine-readable `workflow_hold_reason` stating why work is paused; explanatory prose or a next-review note may supplement but does not replace that field. The Project Integrity Bot reports an omission rather than inventing the reason. Reserve `Completed within scope` for work whose defined obligation is actually complete, not as a generic merger, integration, retirement, archival, or publication label.
