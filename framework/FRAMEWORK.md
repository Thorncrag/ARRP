...
Current area, issue, lifecycle-status, milestone, roadmap, and horizon-tracking status is maintained in the [ARRP GitHub Project](https://github.com/users/Thorncrag/projects/2). [`../areas/README.md`](../areas/README.md) and area README files provide human-readable repository indexes, while developed issue pages contain the substantive analysis. Area pages use a uniform table-based index: active proposals and unresolved candidates appear first in a `Record | Proposal` table; identifiers adjudicated before development appear only in a compact `Record | Disposition` table explaining numbering gaps; and later-retired developed proposals retain direct links in a separate disposition table. The detailed convention is maintained in the [Area-Page Issue Lists standard](METHODOLOGY.md#area-page-issue-lists). Do not duplicate current area lists, issue lists, or developed-issue status snapshots in this framework file unless the list is generated from the GitHub Project or current area pages.
