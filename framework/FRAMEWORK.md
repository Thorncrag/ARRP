...
- [`SOURCE_MONITOR_LOG.md`](logs/SOURCE_MONITOR_LOG.md) contains stable-coded material-change events proposed by deterministic source and directive watchers; routine no-change checks remain in GitHub Actions.
...
- [`PROJECT_INTEGRITY_REPORT.md`](logs/PROJECT_INTEGRITY_REPORT.md) is the overwritten current findings snapshot produced by the integrity checker; it is not a running audit log.
...
- [ARRP Project Console](../research/horizon-review-console/index.html) — internal progress, action-item, candidate, source, monitoring, integrity, project-log, and publication-planning interface
...
- [`SOURCE_MONITOR_LOG.md`](logs/SOURCE_MONITOR_LOG.md) — material deterministic watcher changes and originating workflow runs
...
- [`PROJECT_INTEGRITY_REPORT.md`](logs/PROJECT_INTEGRITY_REPORT.md) — overwritten current integrity findings snapshot
...
- [`../participate/README.md`](../participate/README.md) — public-interaction service, route, privacy, and deployment boundary
...
Current area, issue, lifecycle-status, milestone, roadmap, and horizon-tracking status is maintained in the [ARRP GitHub Project](https://github.com/users/Thorncrag/projects/2). [`../areas/README.md`](../areas/README.md) and area README files provide human-readable repository indexes, while developed issue pages contain the substantive analysis. Area pages use uniform bulleted indexes: active proposals and unresolved candidates appear first; identifiers adjudicated before development appear only in a compact disposition list explaining numbering gaps; and later-retired developed proposals retain direct links in a separate disposition list. The entire identifier-and-title label links to the standalone issue page when one exists. The detailed convention is maintained in the [Area-Page Issue Lists standard](METHODOLOGY.md#area-page-issue-lists). Do not duplicate current area lists, issue lists, or developed-issue status snapshots in this framework file unless the list is generated from the GitHub Project or current area pages.
...
[`../topics/`](../topics/) supplies a selective reader-oriented layer for major subjects of public interest that span multiple institutional failures, areas, or remedies and materially benefit from explanation. [`../topics/README.md`](../topics/README.md) is the public **Explore by Topic** index displayed beside the Subject and Institution Index at the project front door. Each topic page briefly explains the subject, connects common public concerns to the proposals that address them, identifies materially related ideas the project has formally decided not to pursue, and distinguishes institutional defects from ordinary political disagreement. Detailed diagnosis, evidence, remedies, legislation, scores, audits, status, budgets, priorities, and development tasks remain in their designated project records. Public topic pages use familiar titles, plain-language headings, and direct explanations rather than internal routing terminology. Deferred records remain ordinary live routes, while merged or integrated records appear only through their current homes. Each subject has one canonical page; an existing ARRP crosswalk selected for topic treatment is converted and moved rather than duplicated. Admission, required sections, the verbatim-transfer ownership test, rejection handling, source discipline, and concision rules are maintained in the [Topic Page Standard](METHODOLOGY.md#topic-page-standard).

The [Project Console progress view](PROJECT_CONSOLE_PROGRESS.md) may visualize Project data and calculate portfolio-level pace or forecasts, but it is not an independent workflow authority and must not silently repair or override Project fields.

The public website is a generated GitHub Pages artifact, not a publication of the repository tree. [`../website/README.md`](../website/README.md) defines its two-gate allowlist: a canonical Markdown page must carry `public-proposal` metadata and must also be an approved root page or live under `areas/`, `legislation/`, or `topics/`. [`../scripts/prepare_public_site.py`](../scripts/prepare_public_site.py) stages that corpus, converts links to excluded internal Markdown into plain text, generates synchronized topic, area, and legislation navigation, validates that the Project Console and other internal apparatus are absent, and writes an auditable manifest.

Project updates must keep the GitHub Project, issue-page audit metadata, issue audit-history files, Horizon Scan Log, applicable autonomous-run Agent Audit Log entries, source inventory, and reader-navigation surfaces current. When an area or issue is created, removed, renamed, moved, merged, retired, promoted, or materially rerouted, update the affected project-area or area-level contents page and [`../SUBJECT_INDEX.md`](../SUBJECT_INDEX.md) in the same change, as well as the root [`../README.md`](../README.md) if its front-door routes are affected. When a legislation file, audit status, quality score, cited source, indexed organization, or indexed subject is added, removed, renamed, or materially revised, update the relevant GitHub Project item or fields, issue page, sibling audit-history file, [`../inventory/sources.csv`](../inventory/sources.csv), contents page, and subject-index route when each surface is affected. T1 must verify contents-and-index synchronization as part of project integration; T0 flags obvious drift for existing stable records but does not index an unadmitted Horizon candidate. A Change Audit records proposal-specific findings in the affected sibling audit-history file; a durable cross-project lesson is incorporated in the governing file, script, or test that owns it. Do not maintain a new cumulative Change Audit or project-consistency ledger; retain the existing historical records read-only. The integrity bot may overwrite [`PROJECT_INTEGRITY_REPORT.md`](logs/PROJECT_INTEGRITY_REPORT.md) as a current snapshot because it does not accumulate audit history. [`CURRENT_AUDIT.md`](CURRENT_AUDIT.md) is the temporary handoff record, and the committed change history preserves transaction provenance. When a `HOR-###` candidate is added, adjudicated, integrated, retained, or retired, update [`HORIZON_SCAN_LOG.md`](logs/HORIZON_SCAN_LOG.md) and the corresponding GitHub Project item as part of the same change. When an autonomous, batched, or scheduled agent run commits work, update [`AGENT_AUDIT_LOG.md`](logs/AGENT_AUDIT_LOG.md) as part of that run. Deterministic source-watcher changes instead receive one stable-coded entry in [`SOURCE_MONITOR_LOG.md`](logs/SOURCE_MONITOR_LOG.md) within the watcher pull request and are not duplicated in the Agent Audit Log. Human-invoked audit or drafting sessions should not update the Agent Audit Log merely because an agent assisted the work.
...
Every publication-controlled Markdown page must carry either one or more `print_levels` or `print_status: excluded` with a `print_exclusion_reason` under [`PRINT_ASSEMBLY.md`](PRINT_ASSEMBLY.md#print-assignment-metadata). The full technical edition is curated rather than a repository archive. Tool-discovered control files such as root `AGENTS.md` and website-only assets such as `website/404.md` are exempt.

[`print-assembly.json`](print-assembly.json) provides the machine-readable edition-section and ordering structure used by the Project Console and future export tooling. It complements rather than replaces page-level inclusion metadata or the reader-facing rules in [`PRINT_ASSEMBLY.md`](PRINT_ASSEMBLY.md).
