...
Current area, issue, lifecycle-status, milestone, roadmap, and horizon-tracking status is maintained in the [ARRP GitHub Project](https://github.com/users/Thorncrag/projects/2). [`../areas/README.md`](../areas/README.md) and area README files provide human-readable repository indexes, while developed issue pages contain the substantive analysis. Area pages list active proposals and unresolved candidates first; identifiers adjudicated before development appear only in a compact disposition table explaining numbering gaps, while later-retired developed proposals retain direct links to their substantive pages. The detailed convention is maintained in the [Area-Page Issue Lists standard](METHODOLOGY.md#area-page-issue-lists). Do not duplicate current area lists, issue lists, or developed-issue status snapshots in this framework file unless the list is generated from the GitHub Project or current area pages.
