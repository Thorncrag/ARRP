...
Before any stage, the coordinator records the baseline commit and verifies current `main`, authenticated access required by the due stages, no incompatible `Open`, `Paused`, or `Blocked` interactive handoff, and a clean worktree. `CURRENT_AUDIT.md` identifies unfinished continuation state only and never proves that an agent, bot, chain, task, or process is live. Runtime liveness comes from current dispatcher, task, and workflow state together with the coordinator's local dispatch lock, which records the dispatcher process, current Chain ID, invocation, Elim task when known, preserved output path, and a refreshed heartbeat. A lock may be recovered only when its recorded owner is no longer alive, or when a legacy ownerless lock exceeds the reviewed stale-lock interval. Recovery marks Elim failed, preserves the task and JSONL output as incomplete evidence, creates a human Action Item and notification, projects the error to the Console, and requires a fresh current chain before substantive work resumes.
