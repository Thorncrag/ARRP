...
3. Repository gates — only pull requests explicitly typed by the complete
   ...
   append-only declaration authority is
...
Each change is a heading-based structured record rather than a table row. As a
general rule, same-day work in the same change area is collapsed into one
record, including rapid review refinements and related technical repairs. A new
change ID is required only for a materially distinct purpose, review decision,
or rollback boundary; commits and conversational turns do not independently
define log entries.
