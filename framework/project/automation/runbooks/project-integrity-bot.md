...
The deterministic repository baseline is
[`scripts/audit_project_consistency.py`](../../../../scripts/audit_project_consistency.py).
Run it in the authenticated host context when GitHub Issue or Project
synchronization is in scope. A repository-only result does not verify those
hosted surfaces. Run the ordinary test suite and public-site preparation as
complementary validation, sequentially rather than concurrently because both
may rebuild the same staged artifact. External-source availability is a
separate Source Checker responsibility; access controls, moved publications,
and bot blocking must not be misclassified as internal project defects.
...
The encoded floor covers Markdown heading anchors; area `issue_count`,
active-list placement, canonical labels, and links; issue, audit, legislation,
topic-guide, source-catalog, print-assignment, and print-assembly metadata;
reciprocal issue-to-legislation routing; lifecycle-field coherence and required
workflow explanations; front-door discovery routes; structured JSON and CSV
validity; and common tracked-file or local-path artifacts. Its internal-link
pass covers active project Markdown and HTML plus main-branch ARRP repository
URLs stored in active project text records. Generated build output and
superseded archival snapshots are excluded.
...
The current Markdown report is a replaceable snapshot, not a cumulative audit
ledger. Timestamps and bounded history remain in the Console feed and GitHub
Actions. The Integrity screen groups findings for repair; it is not an audit
tier, score, or authority for automatic content changes. A clean automated
result means only that the encoded checks found no defect.
The report builder treats a change in the deterministic finding set,
checked-page counts, or declared check scope as a changed snapshot eligible
for the narrow report proposal.
