...
status: active
authority_scope: "ARRP edition selection, section order, page metadata, exact source and supporting-material treatment, and compiled-export validation."
...
# ARRP Print Assembly Configuration

This file applies the
[Print and Edition Assembly Standard](../../standards/publication/print-assembly.md)
to ARRP's exact page metadata, section order, appendices, and generated
locators. It defines how the modular Markdown project should be assembled into
a single proposal document for printing, PDF generation, DOCX export, or other
linear publication.
...
### Evidence and Source-Development Metadata

ARRP reader-facing issue evidence records live at
`areas/<AREA>/evidence/<ISSUE-ID>-evidence.md` and use this project metadata
shape in addition to any ordinary page metadata:
...
Reader-facing issue evidence records normally carry
`print_levels: [public-proposal]`. An edition may place selected evidence
records in Appendix C or omit them under its explicit plan; eligibility does
not require inclusion in every edition. If a record is deliberately not
eligible for a reader edition, keep it available online and mark it
`print_status: excluded` with a concise reason. An evidence record is never
treated as an independent proposal.
...
The [ARRP source workflow](../workflows/source-adjudication.md#arrp-evidence-and-source-development-paths)
owns the exact record paths, catalog relationship, and qualitative placement
decision. This file owns only their print and edition treatment.
...
Roadmap, backlog, and to-do items are maintained in the GitHub Project and Issues under the [`ARRP GitHub Workflow`](../github/workflow.md). This file records print-assembly rules and does not maintain a separate roadmap.
