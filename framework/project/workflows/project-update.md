...
Benjamin's shorthand `Sync everything` authorizes complete project-wide
operational reconciliation through the ordinary reviewed workflow. The
[Project-Wide Reconciliation policy](../automation/project-wide-reconciliation.json)
specification defines the required estate, dispositions, readbacks, and
completion gate. Unknown or pending state prevents completion. The shorthand
has no content or editorial-neutrality meaning and grants no cleanup,
host-state, control-bypass, pause-removal, or guessed-disposition authority.
...
18. If the update is a project-wide reconciliation, require a fresh local
    inventory, exact owner-local dispositions for every retained state, a
    current authenticated hosted readback bound to the same `origin/main`, and
    a successful deterministic reconciliation result. A primary-checkout-only
    `git status` is insufficient.
