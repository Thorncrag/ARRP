...
Admission and permanent disposition are human-reserved. Only a record-specific human decision may admit, reject, retire, remove from active scope, terminally merge or integrate, or otherwise permanently dispose of a candidate or issue. Standing, class-wide, batch, or runbook authorization is insufficient. Agents may investigate and recommend a disposition; deterministic bots may only supply observations or verify whether the required human decision is recorded within their runbook authority. An agent may implement an already recorded record-specific human decision within its other authority. Implementation must preserve the substantive historical record rather than delete it.

A tracking-only child created solely under the superseded proposal-monitoring convention is an erroneous administrative artifact for this purpose and may be deleted after `needs: monitoring` is confirmed on the existing parent. Its administrative child text need not be migrated because the parent-level pass reviews all associated sources and searches for new developments. This narrow exception does not authorize deletion of a substantive proposal, candidate, evidence, contributor, or adjudication record.

After a record-specific human decision admits a Horizon candidate independently, convert its existing issue into the area-specific proposal issue: assign and retitle it with the new `AREA-###` identifier, replace `kind: horizon` with `kind: proposal`, preserve its original `HOR-###` provenance in the issue body and Horizon Scan Log, update the registry row in place, and keep the issue open. Do not create a duplicate proposal issue. After a record-specific human decision merges, integrates, retires, or rejects a Horizon candidate, retain the `HOR-###` prefix, append the precise bracketed disposition, add a dated final-disposition section that supersedes active-intake language, synchronize the registry and Horizon Scan Log, close the issue if no independent work remains, and remove only its Project card. Read back all affected surfaces before closeout.
...
The canonical [`Issue-Admission Test`](../../FRAMEWORK.md#issue-admission-test), four-part [`Human-Governed Foundation and Delegated Development`](../../FRAMEWORK.md#human-governed-foundation-and-delegated-development) rule, and [`Post-Admission Development Gates`](../../FRAMEWORK.md#post-admission-development-gates) are maintained in the Framework. Agent execution belongs in [`AGENT_OPERATING_RULES.md`](../../AGENT_OPERATING_RULES.md) and the runbooks registered in the [Component Registry](../../component-registry.json). This section implements those judgments through GitHub maturity, workflow, and audit-control fields; it does not redefine them. For an unscored proposal, the canonical page's `foundation_status`, `foundation_approved_date`, and concise optional `foundation_approval_note` preserve the human decision or recorded Elim or interactive-agent sufficiency determination supporting `Development level: In development`. Workflow `Status` alone does not establish that foundation.

Use the Project `Development level` options as follows:

- `Candidate` — a formal active `HOR-###` candidate has not received the record-specific human admission decision required by the canonical Issue-Admission Test.
- `Admitted / undeveloped` — an area-specific proposal has been admitted through a recorded human decision, but its canonical four-part foundation remains unresolved.
- `In development` — the Framework's four-part foundation has been established and recorded; development may proceed within it.
- `Developed proposal` — the Framework's package-completeness gate has been satisfied, or a completed T-audit has assigned a score below Review Ready.
- `Review ready` — the Framework's Review Ready gate has been satisfied.
- `Release candidate` — the Framework's Release candidate gate has been satisfied, including any required recorded human acceptance of departure from the preferred standard. A score alone does not establish this level.
...
- `Human decision needed` — a specific human-reserved decision is the next action, including record-specific admission or permanent disposition, a material foundational choice, the human-only reversed-control decision, or publication authorization. State the exact question and why an agent may not decide it. Do not use this Status when Elim or an interactive Codex agent has authority to determine four-part foundation sufficiency from the existing canonical record.
...
- `Publication approval` — a release candidate is ready for the final human go/no-go decision authorizing circulation or publication. Assignment of this Status does not supply that authorization. Do not use it as a general production, editing, or publication-development queue.
...
At closeout, synchronize both fields by assigning the highest Framework Post-Admission Development Gate supported by the canonical record. An admitted issue without a recorded foundation remains at `Development level: Admitted / undeveloped`; an incomplete package with an established foundation remains at `Development level: In development`; and a package satisfying the Framework's package-completeness gate uses `Development level: Developed proposal`. Use `Status: Audit needed` only when the audit-readiness gate is also satisfied. After a completed T-audit, update the development level under the Framework gates and use Status for the actual next action. `Development level: Review ready` ordinarily uses `Status: External review`. `Development level: Release candidate` uses `Status: Publication approval` only when the final human go/no-go decision is next; the classification and Status do not authorize publication. Research, drafting, source development, status review, Change Audits, and other non-T-audit work do not change `Score` or increment `Runs`.
...
Map issue-page rebaseline metadata to the GitHub Project
`Rebaseline status` field exactly:
...
Individual external sources may independently warrant recurring checking,
especially live dockets, rolling agency pages, or other changing official
records. Record that source-level fact through the exact catalog fields and
baseline semantics in
[ARRP Source Catalog and Adjudication](../workflows/source-adjudication.md#canonical-source-catalogs).
A monitored source neither applies the issue label nor narrows an issue-wide
monitoring pass.
...
The authoritative deterministic monitoring configurations and authority boundaries are maintained in the [Case Monitor Bot](../automation/runbooks/case-monitor-bot.md) and [Presidential Directives Bot](../automation/runbooks/presidential-directives-bot.md) runbooks. Their production implementations run only as stages within the serialized owner-local coordinator and have no standing bot branch. A stage cannot create a branch, commit, pull request, or merge. When an authorized run produces a validated material repository change, the coordinator may include that exact change in its run-bound transaction worktree and deterministic broker path; any resulting transaction branch and owner-assigned pull request remain bound to that run and require the ordinary owner review. A material action records both the applicable domain event and shared Agent Audit Log provenance. A no-change run remains only in the retained run history and bounded Console history.
