...
- Research work product is not a second proposal corpus: its analyses and tools may support an issue or topic guide, but the canonical public treatment remains on the owning issue or topic page. An issue-specific research record must be linked from its owning issue or area page. The root research README identifies cross-project products and routes readers to area research. The internal ARRP Project Console is a non-authoritative view of Review Ready progress, preliminary candidates, GitHub-authoritative formal candidates, both canonical source catalogs, the presidential-directives registry, proposal or formal-candidate issues carrying `needs: monitoring`, and every page's compiled-edition assignment. Its second primary tab, Action Items, is a linked inbox assembled from those owning records. Its Publication workspace can stage and export print-level additions or removals for later Codex validation without editing front matter; its Watchers workspace provides Court Cases, Presidential Directives, and Manual Monitoring views without replacing their owning CSV, registry, GitHub, or front-matter records.
...
- The `project-console-data` branch retains only generated progress and history JSON for the internal Project Console; it contains no duplicate reader-facing dashboard and remains excluded from the website artifact.
