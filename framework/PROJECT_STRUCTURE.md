...
- `.github/workflows/monitoring-pilot.yml`, `.github/monitoring-pilot.json`, and `scripts/check_monitoring_updates.py` implement the deterministic signal-only monitoring prototype. Each run discovers the complete live `needs: monitoring` set from GitHub, matches permanent source identifiers from both source catalogs, checks sources supported by an available adapter, and reports the remainder as requiring manual review. Persistent comparison state remains in one rolling comment on each monitored GitHub issue, while each completed run posts a concise notice to GitHub issue #317; routine runs create no repository file log, proposal edit, or pull request.
