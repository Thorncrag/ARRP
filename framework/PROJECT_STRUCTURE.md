...
framework/                         Governing methodology, historical records, and operating rules
  ...
  PROGRESS_DASHBOARD.md            Review Ready dashboard governance and calculation rules
...
- Research work product is not a second proposal corpus: its analyses and tools may support an issue or topic guide, but the canonical public treatment remains on the owning issue or topic page. An issue-specific research record must be linked from its owning issue or area page. The root research README identifies cross-project products and routes readers to area research. The internal Candidate and Source Intake console is a read-only view of preliminary candidates, GitHub-authoritative formal candidates, both canonical source catalogs, the presidential-directives registry, and proposal or formal-candidate issues carrying `needs: monitoring` together with all sources associated with each. Its Watchers workspace provides Overview, Court Cases, Presidential Directives, and Manual Monitoring views without replacing their owning CSV, registry, or GitHub records.
...
- `.github/workflows/case-monitor-bot.yml`, `.github/case-monitor-bot.json`, and `scripts/check_case_updates.py` implement the scheduled `case-monitor-bot`, which runs daily at approximately midnight Eastern and retains manual dispatch. It retrieves the Just Security Trump-administration litigation tracker once, validates and compares stable row-level case records with a compressed central baseline, and reports added, changed, removed, or structurally invalid results to GitHub issue #317. Only changed rows with supported CourtListener links are eligible for paced, capped REST verification; `COURTLISTENER_API_TOKEN` is optional, and a missing token leaves those changes visibly unverified without preventing the tracker comparison. The bot maps docket identities to affected `SRC-####` and ARRP records where possible, may add a temporary human-review signal to an affected monitored issue, and remains repository-read-only. It does not treat the tracker as complete, make legal judgments, create candidates, change canonical source records, or replace the project-wide human monitoring pass.
- The presidential-directives watcher is a scheduled, read-only comparator that also permits manual dispatch. It may discover, normalize, deduplicate, relate, and detect changes in official presidential instruments; it reports every run to the designated operational issue but does not write the canonical registry. LLM-assisted review owns project relevance, issue routing, preliminary-candidate synthesis, deferral, and no-action dispositions. Any later deterministic registry-writing authority requires explicit authorization plus a dedicated branch and pull request.
...
- The separately deployed `participate/` service is separated from unpublished research tooling and contains no internal source, monitoring, history, or candidate-review queues. It remains outside the Pages artifact by design: its public-input route posts to a canonical GitHub Discussion, its author-contact route sends only to the configured private mailbox, and formal intake review remains manually initiated through Codex. It and the internal Candidate and Source Intake console follow the project-operated interface visual standard in `METHODOLOGY.md`; the canonical GitHub Pages and print themes remain separate.
