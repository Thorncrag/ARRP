...
inventory/                         Structured source and GitHub issue registries
  ...
  sources-pending.csv              Non-citation pending-source catalog
...
- Source records in `inventory/sources.csv` are the relied-upon external-source registry and support assertions in issues, areas, framework files, research files, topic guides, monitoring records, candidate records, or project-level pages. `inventory/sources-pending.csv` holds stable-ID external sources not yet used to support a substantive project assertion.
...
- Research work product is not a second proposal corpus: its analyses and tools may support an issue or topic guide, but the canonical public treatment remains on the owning issue or topic page. An issue-specific research record must be linked from its owning issue or area page. The root research README identifies cross-project products and routes readers to area research. The internal Candidate Issue Intake console is a read-only view of preliminary candidates and GitHub-authoritative formal proposed candidates; source and monitoring work remains attached to its owning issue, evidence record, or GitHub monitor rather than forming parallel console queues.
...
- `.github/workflows/monitoring-pilot.yml`, `.github/monitoring-pilot.json`, and `scripts/check_monitoring_updates.py` retain the currently paused deterministic signal-only monitoring prototype. Both the live Actions workflow and repository configuration are disabled while source coverage is reconsidered. If deliberately re-enabled, each run discovers the complete live `needs: monitoring` set from GitHub, matches permanent source identifiers from both source catalogs, checks sources supported by an available adapter, and identifies records for which no automated check is configured. Persistent comparison state remains in one rolling comment on each monitored GitHub issue, while each completed run posts a concise notice to GitHub issue #317; routine runs create no repository file log, proposal edit, or pull request.
...
- `inventory/sources.csv` remains the relied-upon registry for external sources whether or not a local copy is retained; `inventory/sources-pending.csv` is the separate source-development queue for material not yet used to support an assertion. The repository does not download every registered source merely because it appears in the inventory.
