...
GitHub Issues and the Project Monitoring view are the issue-level monitoring-workflow authority. The ARRP Project Console's **Sources** tab contains a read-only **Watchers and bots** view with **Court Cases**, **Presidential Directives**, and **Manual Monitoring** subviews; do not maintain a duplicative overview screen. It groups monitored matters by accountable ARRP owner and, where useful, by a named case family, executive-action episode, or other factual matter. Every view must distinguish: (1) why the matter is monitored and what development would trigger review; (2) whether a deterministic watcher or a human pass covers it; and (3) its latest known posture. The console does not create, close, edit, or adjudicate an issue. The **Sources** and **Pending sources** subviews expose each source's `Monitoring` value so a changing docket or similar record remains identifiable even when the owning issue is not currently in an issue-wide monitoring pass. Ordinary badges state the complete queue size. A visually distinct update badge states only a genuinely new or changed record awaiting review, using the directive registry status or an unresolved deterministic-watcher pull request; do not use the update color merely to restate a queue total.
...
Reader-facing intake terminology uses **preliminary candidate** for a synthesized question that has no `HOR-###` identifier or GitHub issue, and **proposed candidate** for a formal `HOR-###` record awaiting admission, merger, monitoring, retirement, or another disposition. Preserve **Horizon** in internal technical names, identifiers, logs, metadata, GitHub labels, and workflow instructions. The ARRP Project Console has five primary tabs in this order: **Progress**; **Action Items**, the central linked inbox for review and monitoring work; **Candidates**, containing candidate and preliminary-candidate subviews; **Sources**, containing cited sources, pending sources, watchers, and bot views; and **Publication**, containing the complete print-level page inventory and future publication-planning views. Those views are non-authoritative projections: lifecycle and candidate authority remains in GitHub, bibliographic authority remains in the two source catalogs, presidential-directive identity and review history remains in the directive registry, and print assignment remains in each page's front matter. The Action Items view must route each count to its complete owning view and must not duplicate narrative workflow records. The Publication view may stage per-page additions and removals and export one JSON instruction list for Codex; it does not persist drafts or edit the canonical front matter.
