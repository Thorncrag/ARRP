...
At the start of substantive issue development or any T0-T4 or Change Audit, check the dashboard's unified Sources queue for every task routed to the issue. T0 and T1 may inventory applicable tasks and flag obvious routing defects without undertaking full source integration. Initial development and T2-T4 work should adjudicate applicable tasks to the extent required by that tier: verify the source, test route and remedy fit, make the qualitative placement decision, update `sources.csv` and the reader-facing record when warranted, and remove resolved temporary rows. If a task cannot yet close, preserve it with the exact remaining predicate rather than silently treating the audit as integration.
...
5. **Graduate or remove.** A verified retained external source graduates into one canonical `sources.csv` row and the qualitatively appropriate destination: the proposal page, a linked evidence record, a Horizon or monitoring surface, an active integration-queue row, or registry-only retention supported by an explicit no-additional-reader-value finding. Update an existing row by normalized document identity instead of creating another. Remove a discovery record only after that placement decision is documented. A redundant source adding no evidentiary value and an irrelevant or political-only record may be removed without entering `sources.csv`; Git history supplies rollback and provenance.
