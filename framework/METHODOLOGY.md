...
An area page must present its live work before historical dispositions. Use **Active Issues** for the principal list on every area page and format it as a two-column table headed `Record | Proposal`. Link the stable identifier directly to its standalone issue page when one exists; display the identifier as plain text when no page yet exists. The active table includes unreviewed or source-development candidates as well as developed proposals, because those records still present an unresolved project question. Do not mix finally merged, integrated, retired, rejected, or rerouted records into that table. Do not reproduce volatile lifecycle status, score, priority, audit, or roadmap fields in the area-page index; the GitHub Project remains authoritative for those fields.

When a provisional identifier was adjudicated before it acquired a standalone developed issue page, preserve it below the active list in a compact **Undeveloped Candidate Dispositions** table. The table exists only to explain numbering gaps and route readers to the current home or final disposition. Ordinarily list only `Record` and `Disposition`; consolidate records with the same destination, omit obsolete titles and priorities, and do not reproduce the underlying analysis. The closed GitHub issue, registry, Horizon Scan Log, or destination issue remains the detailed record.

A developed proposal that is later merged, retired, or otherwise made inactive is not merely a numbering gap. Preserve a direct link to its substantive page under **Former Developed Proposals** in a `Record | Disposition` table with a concise disposition and current destination. Do not demote it into the undeveloped-candidate table or delete its page. If an area has no inactive identifiers, omit the empty disposition sections but retain the **Active Issues** heading and table.
