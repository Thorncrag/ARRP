...
3. **Internal source-development record.** When routed sources are not yet appropriate for concise reader-facing prose, cite them in an issue- or candidate-specific internal source-development record. Proposal records live at `areas/<AREA>/research/<ISSUE-ID>-source-development.md`; formal Horizon records live at `research/horizon-source-records/<HOR-ID>-source-development.md`. The record states the proposition or question under review, verification status, and monitoring qualification without treating the material as an adjudicated finding. It is an accountable use of the source and therefore places the source in `sources.csv`; it is not a substitute for later selecting the strongest evidence for the public proposal or evidence page.
...
GitHub Issues and the Project Monitoring view are the issue-level monitoring-workflow authority. The Candidate and Source Intake console's **Watchers** tab is a read-only monitoring workspace with **Overview**, **Court Cases**, **Presidential Directives**, and **Manual Monitoring** views. It groups monitored matters by accountable ARRP owner and, where useful, by a named case family, executive-action episode, or other factual matter. Every view must distinguish: (1) why the matter is monitored and what development would trigger review; (2) whether a deterministic watcher or a human pass covers it; and (3) its latest known posture. The console does not create, close, edit, or adjudicate an issue. The **Sources** and **Pending** tabs separately expose each source's `Monitoring` value so a changing docket or similar record remains identifiable even when the owning issue is not currently in an issue-wide monitoring pass.
...
Reader-facing intake terminology uses **preliminary candidate** for a synthesized question that has no `HOR-###` identifier or GitHub issue, and **proposed candidate** for a formal `HOR-###` record awaiting admission, merger, monitoring, retirement, or another disposition. Preserve **Horizon** in internal technical names, identifiers, logs, metadata, GitHub labels, and workflow instructions. The Candidate and Source Intake console displays separate tabs for proposed candidates, preliminary candidates, cited sources, pending sources, and a consolidated Watchers workspace. Those tabs are read-only projections: candidate and issue-monitoring authority remains in GitHub, bibliographic authority remains in the two source catalogs, and presidential-directive identity and review history remains in the directive registry.
