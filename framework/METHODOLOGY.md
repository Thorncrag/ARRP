...
Before finalizing an audit with repository changes, verify the preservation state: local validation or documented skipped validation, clean or intentionally described working tree, commit created, push attempted when a remote is available, and GitHub issue/Project fields updated and read back when they are in scope. If an environmental approval, authentication, network, or remote error prevents GitHub issue updates, Project updates, readback verification, or push, record the failure in the final audit note and keep `CURRENT_AUDIT.md` active or paused with the exact remaining sync step.
