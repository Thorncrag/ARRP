...
An area page must present its live work before historical dispositions. Use **Active Issues** for the principal list whenever the page also contains inactive record identifiers. The active list includes unreviewed or source-development candidates as well as developed proposals, because those records still present an unresolved project question. Do not mix finally merged, integrated, retired, rejected, or rerouted records into that list.
...
A developed proposal that is later merged, retired, or otherwise made inactive is not merely a numbering gap. Preserve a direct link to its substantive page under **Former Developed Proposals** with a concise disposition and current destination. Do not demote it into the undeveloped-candidate table or delete its page. If an area has no inactive identifiers, the ordinary **Issues** heading remains permissible and no empty disposition section is needed.
