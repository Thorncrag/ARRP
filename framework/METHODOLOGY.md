...
- [`sources.csv`](../inventory/sources.csv) — external sources actually cited in ARRP records.
...
8. If an external source is newly cited, removed, or used for a materially different proposition, add or update it in [`sources.csv`](../inventory/sources.csv). A source that is only queued for verification, monitoring, or possible placement belongs in [`sources-pending.csv`](../inventory/sources-pending.csv), not the citation catalog. When a pending source is actually cited, move its stable row into `sources.csv` and remove it from the pending file.
...
2. **Relevant Proposals** — a concise two-column table headed `Public concern | Applicable proposals` that links each concern to the proposals that address it;
3. **How Concerns Map to Proposals**, when useful — a compact three-column table headed `Public concern | Applicable proposals | How ARRP addresses it`, with one sentence per row and no development-task list;
4. **Related Ideas Not Included**, only when the project has finally rejected, retired, or held outside scope a topic-related candidate, with its stable identifier or concept, one-sentence reason, and a link to the authoritative record; and
5. **What ARRP Does and Does Not Address** — a short statement distinguishing in-scope institutional defects from ordinary policy or political disagreement.
...
Use `Public concern`, not `Reader concern`, because the table describes the publicly recognizable question rather than an attribute of an individual reader. Use `Applicable proposals`, not `Authoritative ARRP route`, `ARRP route`, `Record`, or `Function`. The linked issue and proposal records remain authoritative; the topic page only identifies which ones apply.

The **Related Ideas Not Included** section is a discovery aid, not a second disposition log. Include only final adverse decisions materially connected to the topic; reproduce neither the full Horizon Scan analysis nor every rejected project idea. The Horizon Scan Log, closed GitHub issue, or other designated record remains authoritative. Do not list a deferred or parked record separately: it remains an ordinary live route unless and until finally adjudicated. Do not list merged or integrated records separately: route the reader to the current authoritative home through **Relevant Proposals** or **How Concerns Map to Proposals**. If no topic-related rejection, retirement, or outside-scope decision has been formally recorded, omit the section rather than announcing that none exists. General scope boundaries may be stated separately but must not be presented as individualized rejection decisions.
...
Large discovery catalogs, routing ledgers, preliminary-candidate tables, generated worklists, and similar intake files are temporary work queues rather than permanent source registries. [`../inventory/sources.csv`](../inventory/sources.csv) is the sole canonical registry of external sources retained after adjudication. Git history preserves superseded queue states; the current repository tree should not retain resolved intake rows, promotion tombstones, or duplicate bibliographic records merely to show that a record once existed.
...
The evidence-page decision is qualitative, not quantitative. There is no source-count or episode-count threshold, and neither a large intake nor an additional source automatically requires a separate page. First ask whether the issue page already establishes each material proposition with sufficiently strong, representative evidence. A stronger or necessary new source belongs on the issue page, replacing weaker support where appropriate. Once the issue page is sufficiently evidenced, place additional material in a linked evidence record only when the material has meaningful reader-facing value and the separate record improves clarity, organization, or continuing monitoring. Cumulative corroboration that adds no material reader-facing value may remain in `sources.csv` with an explicit adjudication note rather than generating a page.

For an automated large-intake review, each retained source routed to an existing issue must receive the same qualitative placement decision before the applicable source task is removed: issue-page integration when it strengthens necessary support; linked evidence-record integration when the issue page is already sufficiently supported and the additional material warrants reader-facing treatment; source-registry retention with an explicit no-additional-reader-value finding when it is merely cumulative; or continued placement in the Sources queue when verification or issue development remains incomplete. An association in `sources.csv` alone is not completed reader-facing integration unless that express no-additional-reader-value finding has been made. When a monitor or evidence record exists, the canonical issue page must place one concise **Supporting Record and Updates** subsection at the end of **Manifestation of the Failure**. Its links are labeled **Additional supporting record** and **Watching for updates**; omit either missing target and do not create placeholders or boilerplate on unrelated issue pages.

Do not create an evidence page merely to receive adjudicated material, and do not manufacture a placeholder issue page. When the receiving issue is undeveloped, lacks a canonical issue page, or the episode still requires verification before reader-facing placement, place the episode in [`../research/existing-issue-evidence-integration.csv`](../research/existing-issue-evidence-integration.csv). That machine-readable record supplies evidence-placement tasks to the dashboard's unified Sources queue without duplicating the external-source registry. Remove its row only after the qualitative placement decision is completed. Organize any evidence page by institutional manifestation or mechanism rather than source order. Prefer prose headings and concise bullets over wide source tables that render poorly on mobile or in print.

### Sources-Queue Reconciliation During Issue Work

At the start of substantive issue development or any T0-T4 or Change Audit, check the dashboard's unified Sources queue for every task routed to the issue. T0 and T1 may inventory applicable tasks and flag obvious routing defects without undertaking full source integration. Initial development and T2-T4 work should adjudicate applicable tasks to the extent required by that tier: verify the source, test route and remedy fit, make the qualitative placement decision, move a source into `sources.csv` and the reader-facing record when it is actually cited, and remove resolved temporary rows. If a task cannot yet close, preserve it in `sources-pending.csv` with the exact remaining predicate rather than silently treating the audit as integration.

Queue reconciliation is part of the issue-development or audit work and does not create a separate T-audit run. Corroborating or cumulative-source placement that does not alter the issue theory requires no Change Audit. A source that materially changes a developed issue's manifestation, diagnosis, damage theory, remedy, proposal vehicle, or score basis triggers the ordinary Change Audit and Internal Remedy-Fit rules. At closeout, regenerate the source dashboard and confirm that resolved tasks disappeared, unresolved tasks retain an intelligible next action, and any task awaiting an external event is in Monitoring rather than Sources. When an active proposal has a coherent recurring external watch, reconcile that watch to its `ISSUE-ID-MON` GitHub sub-issue and its evidence record or explicit registry-only placement; do not leave it discoverable only through the intake dashboard.
...
The intake console may aggregate monitoring records and link to their GitHub homes, but GitHub Issues and Project remain the workflow authority. Do not use console controls or local dashboard state to create, close, or adjudicate a monitor.
...
`input records = issue-page integrations + linked evidence-record integrations + registry-only no-additional-value findings + pending integration tasks + Horizon or monitoring integrations + rejected or redundant records + unresolved records`.

Before deleting temporary rows, verify that every retained external source has exactly one canonical source-inventory home and a documented qualitative placement; every retained existing-issue episode has an issue-page destination, a justified linked evidence record, an express registry-only finding, or an active integration row; every reader-facing proposition has a source; no normalized source URL or document identity was duplicated; no dangling temporary candidate identifier remains; litigation posture is labeled accurately; and affected internal links resolve. A batch report should state records processed, distinct episodes, canonical sources added or updated, duplicates consolidated, issue-page integrations, evidence records created and linked, registry-only findings, pending integration tasks, anchor citations selected, monitoring items, rejection categories, preliminary candidates, material issue changes recommended, and unresolved questions. Commit complete batches separately enough to permit rollback without restoring unrelated completed work.
