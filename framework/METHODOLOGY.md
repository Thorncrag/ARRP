...
A **Project Consistency Audit** is a non-tier, non-scoring review of cross-project structure and integration. It may check and mechanically correct file paths, front matter, identifier alignment, heading levels, required-section order, navigation, registries, source numbering, audit-history nomenclature, public-site boundaries, and drift between an already-authoritative repository record and GitHub Project fields. It does not increment **Runs**. Run [`../scripts/audit_project_consistency.py`](../scripts/audit_project_consistency.py) as the repeatable structural and reader-language baseline, then investigate and resolve or route every reported finding; run the ordinary test suite and public-site preparation as complementary validation.
