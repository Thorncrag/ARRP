...
Create an issue evidence record when a pattern depends on more verified evidence than the proposal can present concisely, continuing litigation or events require an organized monitoring record, a large intake routes to the issue, or separating the record materially improves readability. For an automated large-intake review, retained evidence routed to a developed issue must be integrated into a linked evidence record before the applicable integration task is removed. An association in `sources.csv` is registry metadata, not completed reader-facing integration. The canonical issue page must contain a concise link to its evidence record.

Do not create empty evidence pages for every proposal or manufacture a placeholder issue page merely to receive evidence. When the receiving issue is undeveloped and lacks a canonical issue page, place the episode in [`../research/existing-issue-evidence-integration.csv`](../research/existing-issue-evidence-integration.csv) until the issue is developed. That agent-facing queue records the episode, canonical source identifier, intended route, and remaining integration work; it does not duplicate the external-source registry and does not appear in the user-facing Horizon console. Remove its row only after the retained evidence is integrated into a linked issue evidence record or is affirmatively found to add no evidentiary value. Organize evidence pages by institutional manifestation or mechanism rather than source order. Prefer prose headings and concise bullets over wide source tables that render poorly on mobile or in print.
...
5. **Graduate or remove.** A verified retained external source graduates into one canonical `sources.csv` row and the applicable linked evidence record, proposal, Horizon record, or monitoring surface. Update an existing row by normalized document identity instead of creating another. If the receiving issue is undeveloped, transfer the episode-level task to `existing-issue-evidence-integration.csv`; do not treat the source association as completed integration. Remove a discovery record only after the retained source has both a canonical registry home and a reader-facing destination or an active undeveloped-issue integration-queue row. A redundant source adding no evidentiary value and an irrelevant or political-only record may be removed without entering `sources.csv`; Git history supplies rollback and provenance.
...
The agent may automatically normalize, cluster, verify, reroute, update `sources.csv`, maintain and link an evidence record, consolidate corroboration, assign a monitoring disposition, transfer undeveloped-issue work into the existing-issue integration queue, and remove genuinely resolved temporary records when the result is high confidence and does not materially change a developed proposal's theory or remedy. The user need not review individual sources. The agent may not count inventory association alone as evidence integration.
...
`input records = linked evidence integrations + undeveloped-issue integration tasks + Horizon or monitoring integrations + rejected or redundant records + unresolved records`.

Before deleting temporary rows, verify that every retained external source has exactly one canonical source-inventory home, every retained existing-issue episode has either a linked evidence-page destination or an active undeveloped-issue integration row, every evidence-page proposition has a source, no normalized source URL or document identity was duplicated, no dangling temporary candidate identifier remains, litigation posture is labeled accurately, and affected internal links resolve. A batch report should state records processed, distinct episodes, canonical sources added or updated, duplicates consolidated, evidence entries created and linked, undeveloped-issue integration tasks retained, anchor citations selected, monitoring items, rejection categories, preliminary candidates, material issue changes recommended, and unresolved questions. Commit complete batches separately enough to permit rollback without restoring unrelated completed work.
