...
Repository placement follows an authorship rule. ARRP-created project-development work product—including analyses, legal memoranda, crosswalks, catalogs, transformed datasets, comparison tables, and project-generated visualizations—belongs in [`../research/`](../research/) regardless of whether it has already been integrated into an issue. External material not created by ARRP belongs in [`../sources/`](../sources/) only when a local copy is useful and legally and practically appropriate for verification, preservation, or backup. The source inventory remains the authoritative citation catalog whether or not an external source is retained locally; citation does not require downloading or committing a local copy.
