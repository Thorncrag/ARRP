...
The scheduled `case-monitor-bot` may assist without replacing the project-wide pass. It performs one daily comparison of the Just Security Trump-administration litigation tracker against a validated row-level baseline. The comparison may identify added, changed, or removed tracker entries and map their docket identities to existing source and ARRP records. It must preserve stable case identity independently of table order or publisher action-cluster labels; distinguish primary case rows from related appeals; retain exact source status and date fields; and fail closed on missing required headers, duplicate identities, malformed rows, or an implausible mismatch between parsed and publisher-declared totals. The tracker is a curated discovery and status source, not a complete docket feed: its exclusions, consolidated matters, selective case families, editorial lag, and missing narratives remain limitations of automated coverage.

CourtListener verification is targeted rather than corpus-wide. When a changed tracker row identifies a supported CourtListener docket, the bot may compare selected REST v4 docket fields subject to a conservative request cap and pacing. A CourtListener API token is optional; without one, the tracker comparison still completes and identifies the changed entry as awaiting primary-docket verification. A new, unmatched, grouped, or unverified tracker entry remains a review signal and may not be admitted, routed, or characterized automatically. Every other source and every active search obligation remains visible as requiring human review rather than disappearing from the pass.

The bot is signal-only. It may maintain a compressed central baseline on the designated operational issue; maintain a concise rolling comment on an affected labeled parent issue that identifies matched `SRC-####` records; report structured tracker-change, docket-verification, or repeated-failure signals; preserve a short-lived Actions artifact; and apply a temporary human-review label. It must not interpret legal significance, decide that a reassessment trigger has been satisfied, revise proposal or evidence prose, remove the issue's monitoring label, close the issue, move a source between inventories, or change a proposal's status, score, audit count, or remedy. The workflow remains repository-read-only, and a no-change run does not append to the Agent Audit Log. Any separately authorized source-writing automation requires a dedicated branch and pull request that identifies each affected stable source ID, action, reason, destination, proposition or citation, and originating run; the same pull request must append an autonomous-change entry to the [Agent Audit Log](logs/AGENT_AUDIT_LOG.md). Removing the temporary review label after substantive review acknowledges the observed fields as the next machine baseline; it does not itself alter the project analysis.
...
The presidential-directives watcher runs on a daily schedule near midnight Eastern Time and remains available for manual dispatch. Each run compares the completed registry baseline against official Federal Register metadata, reports new, changed, unchanged, and not-seen records in the Actions summary and retained artifact, and posts an outcome notice to the designated operational issue. It may generate a proposed registry diff or dry-run report but may not directly change canonical records or make project dispositions. LLM or human review remains responsible for legal significance, relevance, routing, and candidate decisions. Any later write authority must be limited to deterministic registry fields, use a dedicated branch and pull request, preserve substantive review control, and follow the Agent Audit Log rules for autonomous repository changes. A no-change scheduled comparison does not create an Agent Audit Log entry.
