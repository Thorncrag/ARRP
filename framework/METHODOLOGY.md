...
Large source-intake collections must be synthesized before they become a user review queue. Preserve every useful source record and its provenance, including independently valuable duplicate coverage, but do not require the user to adjudicate raw articles, tracker rows, cases, or agency actions one at a time. The source-development pass should instead: (1) route evidence relevant to an existing proposal or active Horizon record into an auditable integration queue identifying its likely use; (2) consolidate records that may expose the same otherwise-unowned institutional weakness into a preliminary candidate; and (3) retain uncertain, open-litigation, or insufficiently verified material for agent review or monitoring. Only the synthesized preliminary candidates should appear in the user-facing intake console. A preliminary candidate has no `HOR-###` identifier and creates no GitHub issue or Project item until the user approves promotion. Promotion creates the formal Horizon candidate for the ordinary duplicate, admission, and adjudication workflow; it does not itself admit an area-specific proposal.
