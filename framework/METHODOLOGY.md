...
5. **Graduate or remove.** A verified retained external source graduates into one canonical `sources.csv` row and the applicable proposal, evidence record, Horizon record, or monitoring surface. Update an existing row by normalized document identity instead of creating another. After successful validation, remove the resolved record from every temporary queue. A redundant source adding no evidentiary value and an irrelevant or political-only record may be removed without entering `sources.csv`; Git history supplies rollback and provenance.
...
The agent may automatically normalize, cluster, verify, reroute, update `sources.csv`, maintain an evidence record, consolidate corroboration, assign a monitoring disposition, and remove resolved temporary records when the result is high confidence and does not materially change a developed proposal's theory or remedy. The user need not review individual sources.
...
`input records = graduated sources + consolidated corroboration + monitoring items + candidate evidence + rejected or redundant records + unresolved records`.

Before deleting temporary rows, verify that every retained external source has exactly one canonical source-inventory home, every evidence-page proposition has a source, no normalized source URL or document identity was duplicated, no dangling temporary candidate identifier remains, litigation posture is labeled accurately, and affected internal links resolve. A batch report should state records processed, distinct episodes, canonical sources added or updated, duplicates consolidated, evidence entries created, anchor citations selected, monitoring items, rejection categories, preliminary candidates, material issue changes recommended, and unresolved questions. Commit complete batches separately enough to permit rollback without restoring unrelated completed work.
