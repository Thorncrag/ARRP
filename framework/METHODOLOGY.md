...
For an automated large-intake review, each retained source routed to an existing issue must receive the same qualitative placement decision before the applicable integration task is removed: issue-page integration when it strengthens necessary support; linked evidence-record integration when the issue page is already sufficiently supported and the additional material warrants reader-facing treatment; source-registry retention with an explicit no-additional-reader-value finding when it is merely cumulative; or continued placement in the integration queue when verification or issue development remains incomplete. An association in `sources.csv` alone is not completed reader-facing integration unless that express no-additional-reader-value finding has been made. When an evidence record is created, the canonical issue page must contain a concise link to it.

Do not create an evidence page merely to receive adjudicated material, and do not manufacture a placeholder issue page. When the receiving issue is undeveloped, lacks a canonical issue page, or the episode still requires verification before reader-facing placement, place the episode in [`../research/existing-issue-evidence-integration.csv`](../research/existing-issue-evidence-integration.csv). That agent-facing queue records the episode, canonical source identifier, intended route, and remaining integration work; it does not duplicate the external-source registry and does not appear in the user-facing Horizon console. Remove its row only after the qualitative placement decision is completed. Organize any evidence page by institutional manifestation or mechanism rather than source order. Prefer prose headings and concise bullets over wide source tables that render poorly on mobile or in print.
...
The preliminary-candidate queue contains only unresolved questions awaiting user action. A **Yes** decision creates a formal `HOR-###`, reroutes supporting evidence to that formal record, and removes the preliminary row. A **No** decision applies the approved rejection or existing-issue disposition and removes the row. A **Defer** decision remains only while a defined research or monitoring predicate is active. Do not retain promoted, rejected, or merged rows in the active preliminary queue or leave evidence routes pointing to obsolete `INTAKE-GAP-*` identifiers.
