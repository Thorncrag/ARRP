...
- [`sources-pending.csv`](../inventory/sources-pending.csv) — external sources retained for source-development, monitoring, or adjudication before they are cited.
...
8. If an external source is newly relied upon, removed, or used for a materially different proposition, add or update it in [`sources.csv`](../inventory/sources.csv). A source that is only queued for verification, monitoring, or possible placement belongs in [`sources-pending.csv`](../inventory/sources-pending.csv), not the relied-upon registry. When a substantive project record actually uses a pending source to support an assertion, move its stable row into `sources.csv` and remove it from the pending file.
...
Large discovery catalogs, routing ledgers, preliminary-candidate tables, generated worklists, and similar intake files are temporary work queues rather than permanent source registries. Every retained external source has one inventory home: [`../inventory/sources.csv`](../inventory/sources.csv) when a substantive ARRP record affirmatively relies on it, or [`../inventory/sources-pending.csv`](../inventory/sources-pending.csv) while it remains a source-development, monitoring, verification, or placement lead not yet used to support an assertion. Git history preserves superseded queue states; the current repository tree should not retain resolved intake rows, promotion tombstones, or duplicate bibliographic records merely to show that a record once existed. Source identifiers are permanent and must never be reassigned or renumbered. A documented removal may therefore leave a gap in the numeric sequence; allocate new identifiers above the highest identifier already used rather than filling a gap.
...
3. **GitHub monitoring wrapper.** When a coherent body of external matters requires recurring review, create one `ISSUE-ID-MON` native source-review sub-issue under the proposal as governed by [GitHub Workflow](GITHUB_WORKFLOW.md#issue-specific-monitoring-records). It owns the GitHub Project work state, not the substantive analysis. A monitored matter with several implications receives one primary analytic home and any number of affected-issue cross-references; do not duplicate the case history or its source record.

Unverified discovery leads, provisional routes, uncertain litigation posture, and unresolved placement work belong in `sources-pending.csv` with an accountable owner and exact next action. Temporary batch files may exist outside the repository while a scan is running, but the active tree must not retain a parallel source, litigation, integration, or routing ledger after reconciliation. Inclusion by a tracker or classifier does not make a lead public evidence.

The evidence-page decision is qualitative, not quantitative. There is no source-count or episode-count threshold, and neither a large intake nor an additional source automatically requires a separate page. First ask whether the issue page already establishes each material proposition with sufficiently strong, representative evidence. A stronger or necessary new source belongs on the issue page, replacing weaker support where appropriate. Once the issue page is sufficiently evidenced, place additional material in a linked evidence record only when the material has meaningful reader-facing value and the separate record improves clarity, organization, or continuing monitoring. Cumulative corroboration that adds no material reader-facing value does not require a page: retain it in `sources-pending.csv` while it remains useful for monitoring or future verification, cite it in a project-authored record when that record genuinely relies on it, or remove it after documenting a no-additional-value disposition.

For an automated large-intake review, each retained source routed to an existing issue must receive the same qualitative placement decision before the applicable source task is removed: issue-page integration when it strengthens necessary support; linked evidence-record integration when the issue page is already sufficiently supported and the additional material warrants reader-facing treatment; a genuine citation in a project-authored research or monitoring record when that record relies on the source; continued placement in `sources-pending.csv` when verification, monitoring, or issue development remains incomplete; or removal after a documented no-additional-value finding. Association with an issue is not a citation and does not justify placement in `sources.csv`. When a monitor or evidence record exists, the canonical issue page must place one concise **Supporting Record and Updates** subsection at the end of **Manifestation of the Failure**. Its links are labeled **Additional supporting record** and **Watching for updates**; omit either missing target and do not create placeholders or boilerplate on unrelated issue pages.

Do not create an evidence page merely to receive adjudicated material, and do not manufacture a placeholder issue page. When the receiving issue is undeveloped, lacks a canonical issue page, or the episode still requires verification before reader-facing placement, retain each relevant external source in `sources-pending.csv`. Its associated-record field identifies the receiving issue, candidate, research record, or GitHub monitor, while its proposition and notes state the placement obligation and remaining predicate. Move it to `sources.csv` only when a substantive record actually relies on it. Organize any evidence page by institutional manifestation or mechanism rather than source order. Prefer prose headings and concise bullets over wide source tables that render poorly on mobile or in print.

### Source Reconciliation During Issue Work

At the start of substantive issue development or any T0-T4 or Change Audit, check every pending source and GitHub monitoring record owned by or cross-referenced to the issue. T0 and T1 may inventory applicable tasks and flag obvious routing defects without undertaking full source integration. Initial development and T2-T4 work should adjudicate applicable tasks to the extent required by that tier: verify the source, test route and remedy fit, make the qualitative placement decision, move a source into `sources.csv` and the reader-facing record when it is actually cited, and remove resolved pending rows. If a task cannot yet close, preserve it in `sources-pending.csv` or the issue-linked GitHub monitor with the exact remaining predicate rather than silently treating the audit as integration.

Source reconciliation is part of the issue-development or audit work and does not create a separate T-audit run. Corroborating or cumulative-source placement that does not alter the issue theory requires no Change Audit. A source that materially changes a developed issue's manifestation, diagnosis, damage theory, remedy, proposal vehicle, or score basis triggers the ordinary Change Audit and Internal Remedy-Fit rules. At closeout, confirm that resolved pending rows were removed, unresolved records retain an intelligible owner and next action, and any task awaiting an external event is attached to an `ISSUE-ID-MON` GitHub record rather than left as an orphaned source task. Rebuild the Candidate Issue Intake console only when preliminary- or proposed-candidate data changed; existing-issue source and monitoring work does not appear as a separate console queue.

### Project-Wide Monitoring Pass

A **project-wide monitoring pass** is a non-scoring review of every open Project item with `Status: Monitoring` or `needs: monitoring`. It is distinct from a T-audit and does not alter proposal scores or audit-run counts merely because an external matter remains open.

For each record, refresh the source or docket posture; compare it with the stated trigger; update the monitor issue's concise body, last-checked date, sources, and reader-facing evidence placement as needed; and close the monitor when its defined obligation is resolved. If a result materially changes an issue's manifestations, diagnosis, remedy, legislative vehicle, or score basis, run the ordinary targeted Change Audit and Internal Remedy-Fit review before treating the parent proposal as current. If the result is cumulative or immaterial, document the no-additional-reader-value finding in the source record without expanding the issue page.

GitHub Issues and the Project Monitoring view are the monitoring-workflow authority. The Candidate Issue Intake console does not aggregate monitoring records, and no local console state may create, close, or adjudicate a monitor.

A deterministic scheduled check may assist the project-wide monitoring pass by discovering every open GitHub issue labeled `needs: monitoring`, matching its permanent `SRC-####` associations from both source catalogs, and comparing every source supported by an available deterministic adapter with the last acknowledged machine baseline. Unsupported sources and predicates remain visible as requiring manual review rather than disappearing from the run. The automation is signal-only: it may maintain one rolling bot comment on the monitored GitHub issue, report the check in the GitHub Actions summary, post a per-run operational notice to the designated GitHub notification issue, and apply a temporary human-review label when a relevant source field changes or a check repeatedly fails. The notice is not a canonical change log. The scheduled workflow must remain repository-read-only and must not decide that a reassessment trigger has been satisfied, characterize the legal significance of a filing or source change, revise proposal or evidence prose, alter a monitoring disposition, close a monitor, move a source between inventories, or change a proposal's status, score, audit count, or remedy. A run that changes no repository record does not append to the Agent Audit Log. Any separately authorized future source-writing automation must work through a dedicated branch and pull request whose body identifies each affected stable source ID, action, reason, destination, proposition or citation, and originating run; the same PR must append an autonomous-change entry to the [Agent Audit Log](logs/AGENT_AUDIT_LOG.md), while the PR diff and merged Git history preserve prior and new values, timestamp, and authorship. Removing the temporary review label after substantive review acknowledges the observed fields as the next machine baseline; it does not itself alter the underlying project analysis.
...
5. **Retain, cite, or remove.** A source kept for verification, monitoring, or future placement receives one stable `sources-pending.csv` row and an accountable owner and route: an issue or evidence record, an issue-linked GitHub monitor, or one preliminary candidate. Move that row into `sources.csv` only when it supports a citation in a proposal page, evidence record, topic guide, or project-authored research record. Update an existing row by normalized document identity instead of creating another. Remove a discovery record only after its disposition is documented. A redundant source adding no evidentiary value and an irrelevant or political-only record may be removed without entering either catalog; Git history supplies rollback and provenance.

Every retained source must leave adjudication with an accountable owner. That owner is (1) an existing issue or its linked evidence record; (2) a defined-predicate GitHub monitor attached to one or more existing issues or formal candidates; or (3) one preliminary candidate representing a plausible distinct institutional weakness not already owned by the project. Do not retain an orphan-source queue or ask the user to review individual source records. If verification remains incomplete but the material plausibly identifies an unowned weakness, state that uncertainty in the preliminary candidate's unresolved questions or defer predicate. If it does not yet support a plausible defect or an existing route, reject or remove it under a documented disposition rather than preserving an ownerless record.
...
Reader-facing intake terminology uses **preliminary candidate** for a synthesized question that has no `HOR-###` identifier or GitHub issue, and **proposed candidate** for a formal `HOR-###` record awaiting admission, merger, monitoring, retirement, or another disposition. Preserve **Horizon** in internal technical names, identifiers, logs, metadata, GitHub labels, and workflow instructions. The candidate console displays only these two stages. Supporting sources appear within their preliminary candidate; existing-issue evidence and monitoring remain with their authoritative issue, evidence record, and GitHub monitor rather than becoming separate console tabs.

Each proposed-candidate view should be a decision dossier assembled from existing authoritative records: GitHub supplies issue and lifecycle data; the Horizon Scan Log supplies intake history, overlap, rationale, and follow-up; the two source inventories supply evidentiary records; and identifier-linked research supplies project analysis. Display missing inputs as record gaps rather than filling them with generated conclusions. The console may cache this derived presentation data, but it must not become a manually maintained narrative ledger or override its inputs; corrections belong in the record that owns the information. The console must remain read-only: no interface control may invoke an agent, record or implement a disposition, or mutate GitHub or canonical project records. Conduct every formal candidate decision within Codex under the full Horizon Candidate Adjudication Workflow.

### Presidential-Directive Completeness Scans

Use a presidential-directive scan as a completeness backstop for litigation, media, and specialist-source intake—not as a presumption that every executive order or presidential document identifies a project issue. The unit of analysis remains the generalized institutional weakness. An executive order, proclamation, memorandum, determination, notice, presidential order, correction, implementing agency action, and related litigation may be parts of one evidence episode; several directives using the same institutional mechanism may support one candidate; and one directive may require one primary analytic home plus affected-record cross-references.
...
Before substantive review, compare each directive against issue and evidence pages, legislation, formal proposed candidates, active preliminary candidates, both source inventories, and GitHub monitoring records. Plainly ceremonial proclamations, routine administration, ordinary policy implementation within delegated discretion, cumulative records adding no evidentiary value, and already-owned directives may receive a documented no-project-action or redundant disposition. Document type or a keyword alone may not decide the question: an operative proclamation, delegation, emergency continuation, policy reversal, or unnumbered order must remain reviewable when it may bypass a safeguard or expose a durable remedial weakness.

Review unmatched operative directives in a temporary batch outside the active repository tree. Apply the ordinary first substantive pass and independent challenge pass, including the political-failure, reversed-party neutrality, durable-harm, duplicate, route-fit, and least-complex-remedy tests. If a plausible distinct institutional weakness remains without an existing owner, the same validated batch must create or update one clustered `INTAKE-GAP-###` preliminary candidate and attach the directive source. Cluster by actor and power used, safeguard bypassed or missing, resulting institutional risk, and likely remedy family—not by directive title alone. If an active preliminary candidate already owns that weakness, pass its identifier explicitly rather than create a semantic duplicate. Uncertainty belongs in the candidate's unresolved question or a defined monitoring predicate; it does not justify an orphan source row.

At closeout, route each retained directive to `sources.csv` or `sources-pending.csv` under an existing issue, evidence record, GitHub monitor, or preliminary candidate; remove the temporary batch; and leave no separate directive queue or console tab. Record aggregate input, match, exclusion, integration, monitoring, candidate, rejection, correction, and retrieval-error counts in a maintained research summary. Later scans begin after the recorded coverage cutoff and must not recreate previously adjudicated directives. The current completed baseline and cutoff are recorded in the [Trump Administration Legal-Review Research Summary](../research/trump-administration-legal-review-summary.md).

### Automation and Human-Decision Boundary

The agent may automatically normalize, cluster, verify, reroute, update the pending or cited source catalog as the source's current use requires, make and document the qualitative placement decision, maintain and link an evidence record where warranted, consolidate corroboration, assign a monitoring disposition, preserve incomplete work in `sources-pending.csv`, and remove genuinely resolved temporary records when the result is high confidence and does not materially change a developed proposal's theory or remedy. The user need not review individual sources. The agent may not create an evidence page merely because material was routed to an issue, use a numeric threshold, or count inventory association alone as evidence integration.
...
If an audit changes an eligible proposal's Project `Status`, `Score`, or goal eligibility, closeout must also refresh the derived Review Ready progress dashboard. After the authoritative Project update has been read back and the audit commit has been pushed, manually dispatch the `Review Ready progress dashboard` workflow, wait for it to complete, and verify the generated [`progress-dashboard/PROGRESS.md`](https://github.com/Thorncrag/ARRP/blob/progress-dashboard/PROGRESS.md) reflects the new portfolio count, status, score movement, area result, or closest-to-ready list as applicable. The daily schedule is a recovery backstop and does not replace same-session verification. An expressly authorized multi-issue or successive-tier batch may use one final refresh after all included Project changes and pushes, provided the generated readback confirms the complete batch. Never edit the generated branch by hand.

Before finalizing an audit with repository changes, verify the preservation state: local validation or documented skipped validation, clean or intentionally described working tree, commit created, push attempted when a remote is available, GitHub issue/Project fields updated and read back when they are in scope, and any required progress-dashboard refresh completed and read back. If an environmental approval, authentication, network, remote, workflow, or publication error prevents GitHub issue updates, Project updates, dashboard refresh, readback verification, or push, record the failure in the final audit note and keep `CURRENT_AUDIT.md` active or paused with the exact remaining sync step.
