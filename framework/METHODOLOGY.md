...
`sources.csv` is the project-wide registry of distinct external sources affirmatively relied upon to support a factual, legal, historical, procedural, monitoring, or analytical assertion in a project-authored substantive record. The supporting record may be reader-facing prose or an accountable structured research, monitoring, evidence-integration, or candidate record. A structured reference counts only when it cites the stable `SRC-####` identifier, states the action or proposition supported, and identifies an accountable issue, candidate, or monitor. Mere issue association, topical routing, raw-intake inclusion, bookmarking, or consultation without a retained assertion does not qualify a source for `sources.csv`; such material belongs in `sources-pending.csv` while potentially useful or is removed after documented adjudication.

Repository placement follows an authorship-and-publication rule. Unpublished ARRP-created project-development work product—including analyses, legal memoranda, crosswalks, catalogs, transformed datasets, comparison tables, and project-generated visualizations—belongs in [`../research/`](../research/) regardless of whether it has already been integrated into an issue. When the project deliberately selects one of those analyses as a canonical public topic guide, move and convert it into [`../topics/`](../topics/) rather than retaining a duplicate research copy. External material not created by ARRP belongs in [`../sources/`](../sources/) only when a local copy is useful and legally and practically appropriate for verification, preservation, or backup. The source inventory remains the authoritative relied-upon source registry whether or not an external source is retained locally; reliance does not require downloading or committing a local copy.
...
### Four-Layer Evidence and Monitoring Architecture
...
4. **Temporary research intake.** Unverified discovery leads, provisional routes, semantic duplicates awaiting reconciliation, uncertain litigation posture, and unresolved candidate evidence remain in temporary research queues only until adjudicated. They do not become public evidence merely because a tracker or classifier included them.
...
A deterministic scheduled check may assist the project-wide monitoring pass by comparing narrowly configured structured-source fields with the last acknowledged machine baseline. The automation is signal-only: it may maintain one rolling bot comment on the monitored GitHub issue, report the check in the GitHub Actions summary, and apply a temporary human-review label when a relevant source field changes or a check repeatedly fails. It must not decide that a reassessment trigger has been satisfied, characterize the legal significance of a filing or source change, revise proposal or evidence prose, alter a monitoring disposition, close a monitor, move a source between inventories, or change a proposal's status, score, audit count, or remedy. Removing the temporary review label after substantive review acknowledges the observed fields as the next machine baseline; it does not itself alter the underlying project analysis.
