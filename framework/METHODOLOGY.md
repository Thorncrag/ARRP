...
15. If proposal development, horizon integration, or a material source update introduces a department, agency, office, court, other institutional body, acronym, alias, or plain-language subject that would help readers find the relevant work, add or revise the corresponding route in the single alphabetical table in [`../SUBJECT_INDEX.md`](../SUBJECT_INDEX.md). Do not divide entries by entity or topic. Alphabetize departments, agencies, offices, and services by their familiar functional names or acronyms rather than generic openings such as `Department of`, `United States`, or `Office of`, retaining formal names and acronyms in the entry. Use concise stable identifiers, place the preferred destination first, and include alternate routes only when they materially improve discovery. For an especially common term readers are likely to seek under a different letter or wording, add a conventional **See** entry pointing to the canonical index term rather than duplicating its identifiers. Do not add individual entries for candidates rejected, retired, or ruled outside scope; preserve them in [`HORIZON_SCAN_LOG.md`](HORIZON_SCAN_LOG.md) and use the index's single general disposition route. If the preferred destination changes, update the canonical entry and all affected **See** references in the same change.
...
5. the **Budgetary Impact Statement** has the substantive statement first and the italicized `Note:` disclaimer below it, or an affected JUD-011 issue has two separately labeled pathway statements followed by one disclaimer;
...
9. **Implement the approved disposition.** If admitted as a new issue, assign the next stable area-specific issue ID, update the area page and `issue_count`, and update the GitHub Project item/fields, [`sources.csv`](../inventory/sources.csv), and [`HORIZON_SCAN_LOG.md`](HORIZON_SCAN_LOG.md). If merged or retained as source development, update the relevant area or issue page, source inventory, GitHub Project item, and Horizon Scan Log. If retired or rejected as outside scope, close or update the active GitHub horizon issue and add an Integration Log row preserving the static `HOR-###` ID, original concern, rationale, and revisit trigger; do not create or retain an individual Subject and Institution Index entry for that candidate. When the disposition ends all active work on the horizon record, close the issue and remove its card from the active Project without deleting the issue.
10. **Maintain the Horizon Scan state.** Active horizon candidates should live as GitHub Issues and GitHub Project items. Adjudicated candidates should appear in the Horizon Scan Log and remain available through their closed GitHub issues; they should not remain on the active Project unless a specific continuing workflow obligation is documented.
...
When an issue page displays a Proposal Quality Score, the threshold status should appear in parentheses immediately after the score, for example: `82 / 100 (Review Ready)`. GitHub Project status fields should use the same threshold-status labels for formula-based proposal scores when practical; active fixed zero-status items may use routing labels such as `Deferred / Parked` or `Pending development` when those labels are more informative. Reserve `Done / Published` for work actually completed or published within its own scope, not as a generic merger, integration, retirement, or archival label.
