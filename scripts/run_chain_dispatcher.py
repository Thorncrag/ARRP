...
FILE_PROVIDER_DOMAIN_XATTR = "com.apple.file-provider-domain-id"
HOST_GIT_IDENTITY = {
...
def read_file_provider_domain(path: Path) -> str | None:
    """Read the fixed macOS File Provider marker without a shell."""
    result = subprocess.run(
        [
            EXECUTABLES["xattrPath"],
            "-p",
    ...
    if result.returncode == 0:
        return result.stdout or "present"
    if result.returncode == 1 and "No such xattr:" in result.stderr:
        return None
    raise RuntimeError(
        "could not verify the canonical workspace storage boundary at "
        f"{path}: {result.stderr.strip() or 'xattr inspection failed'}"
    )


def file_provider_managed_ancestor(repo: Path) -> Path | None:
    """Return the first File Provider-managed boundary containing the repo."""
    for candidate in (repo, *repo.parents):
        if read_file_provider_domain(candidate) is not None:
            return candidate
    return None
...
def verify_canonical_runtime_boundary(
    ...
    *,
    ...
    managed_ancestor = file_provider_managed_ancestor(repo)
    if managed_ancestor is not None:
        raise RuntimeError(
            "canonical ARRP workspace is inside a macOS File Provider domain "
            f"at {managed_ancestor}; use the reviewed non-synced repository path"
        )
    origin = command([git, "remote", "get-url", "origin"], cwd=repo)
    if (
        origin.returncode != 0
