#!/usr/bin/env python3
"""Publish a generated dashboard tree to a dedicated GitHub branch."""
...
USER_AGENT = "ARRP-review-ready-dashboard-publisher/1.0"
...
def collect_files(source: Path) -> List[Tuple[str, str]]:
    if not source.is_dir():
        raise RuntimeError("Dashboard source directory does not exist: {}".format(source))
    files = []
    for path in sorted(candidate for candidate in source.rglob("*") if candidate.is_file()):
        relative = path.relative_to(source).as_posix()
        files.append((relative, path.read_text(encoding="utf-8")))
    if not files:
        raise RuntimeError("Dashboard source directory is empty: {}".format(source))
    return files
...
def publish(source: Path, repository: str, branch: str, token: str) -> str:
    ...
    dashboard_data = json.loads((source / "data" / "dashboard.json").read_text(encoding="utf-8"))
    commit = api_request(
        token,
        "POST",
        "/repos/{}/git/commits".format(repository),
        {
            "message": "Refresh Review Ready progress {}".format(dashboard_data["asOf"]),
            "tree": tree["sha"],
            "parents": parents,
        },
    ) or {}
...
def main() -> int:
    ...
    if not token:
        raise RuntimeError("Missing {} for dashboard publication.".format(args.token_env))
