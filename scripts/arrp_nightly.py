    ...
    *,
    ...
    while True:
        ...
        if (
            ...
            and readback.get("user", {}).get("login")
            == TRUSTED_CONSOLE_PR_ACTOR
            and readback.get("user", {}).get("type") == "Bot"
        ):
            return readback
...
def record_elim_outcome_in_run_chain(
    chain_path: Path,
    chain: Mapping[str, Any],
    elim_result: Mapping[str, Any],
) -> dict[str, Any]:
    """Bind the accepted Elim terminal result into the canonical run record."""

    recorded_at = iso_utc()
    decision = dict(chain.get("elim_decision") or {})
    decision.update(
        {
            "launched": True,
            "outcome": elim_result.get("outcome"),
            "completed_at": recorded_at,
            "result_sha256": "sha256:"
            + file_sha256(chain_path.parent / "elim-result.json"),
        }
    )
    updated = {**chain, "elim_decision": decision, "updated_at": recorded_at}
    atomic_write_json(chain_path, updated)
    return updated
...
def run_production_cycle(
    ...
    if (chain_value.get("elim_decision") or {}).get("launch_recommended"):
        ...
        chain_value = record_elim_outcome_in_run_chain(
            chain,
            chain_value,
            elim_result,
        )
...
def _revision_json(repository: Path, revision: str, path: str) -> dict[str, Any]:
    entry = _tree_entry(repository, revision, path)
    if entry is None:
        raise TransactionError(f"trusted generated output is missing {path}")
    try:
        value = json.loads(git(repository, "cat-file", "blob", entry[1]).stdout)
    except (json.JSONDecodeError, UnicodeError) as error:
        raise TransactionError(f"trusted generated output is malformed: {path}") from error
    if not isinstance(value, dict):
        raise TransactionError(f"trusted generated output is not an object: {path}")
    return value


def _revision_bytes(repository: Path, revision: str, path: str) -> bytes:
    entry = _tree_entry(repository, revision, path)
    if entry is None:
        raise TransactionError(f"trusted generated output is missing {path}")
    return git(repository, "cat-file", "blob", entry[1]).stdout
...
def validate_trusted_console_generation(
    repository: Path,
    head_commit: str,
) -> frozenset[str]:
    """Prove the complete tracked Console bundle came from its closed serializer."""

    registry = _revision_json(
        repository,
        head_commit,
        "framework/component-registry.json",
    )
    try:
        exemption = registry["registration_exemptions"]["entries"][
            "project_console_generated_data"
        ]
        data_scope = registry["directory_scopes"]["entries"]["project_console_data"]
        catalog_scope = registry["directory_scopes"]["entries"][
            "project_console_catalog_data"
        ]
        producer = registry["components"]["entries"]["project_console_builder"]
    except (KeyError, TypeError) as error:
        raise TransactionError("trusted Console Registry binding is unavailable") from error
    if (
        exemption.get("artifact_class") != "generated_output"
        or exemption.get("authorized_producers") != ["project_console_builder"]
        or exemption.get("canonical") is not False
        or exemption.get("independently_controlled_information") is not False
        or exemption.get("scope_id") != "project_console_data"
        or exemption.get("exact_paths") != [TRUSTED_CONSOLE_CATALOG]
        or data_scope.get("path_pattern") != TRUSTED_CONSOLE_DATA_PREFIX
        or data_scope.get("repository_controls")
        != {"github_codeowners": {"mode": "none"}}
        or catalog_scope.get("path_pattern") != TRUSTED_CONSOLE_CATALOG
        or catalog_scope.get("match_kind") != "exact_file"
        or catalog_scope.get("repository_controls")
        != {"github_codeowners": {"mode": "none"}}
        or producer.get("canonical_source") != "scripts/build_project_console.py"
    ):
        raise TransactionError("trusted Console Registry binding is incompatible")

    manifest_path = TRUSTED_CONSOLE_DATA_PREFIX + "generation-manifest.json"
    manifest_bytes = _revision_bytes(repository, head_commit, manifest_path)
    manifest = _revision_json(repository, head_commit, manifest_path)
    canonical_manifest = (
    ...
    if (
        ...
        or re.fullmatch(r"project-console-[a-f0-9]{20}", generation_id) is None
        ...
        raise TransactionError("trusted Console generation manifest is invalid")

    expected_paths = {TRUSTED_CONSOLE_CATALOG, manifest_path}
    expected_files: dict[str, dict[str, Any]] = {}
    for row in domains:
        if not isinstance(row, dict) or set(row) != {
            "file", "sha256", "bytes", "keys", "record_count"
        }:
            raise TransactionError("trusted Console domain record is invalid")
        name = row.get("file")
        if (
            not isinstance(name, str)
            or Path(name).name != name
            or not name.endswith(".js")
            or name.startswith(("private-", "local-"))
            or name in expected_files
        ):
            raise TransactionError("trusted Console domain name is unsafe")
        expected_files[name] = row
        expected_paths.add(TRUSTED_CONSOLE_DATA_PREFIX + name)
    if set(files) != set(expected_files):
        raise TransactionError("trusted Console manifest file index differs")
    ...
    if tracked_data != expected_paths - {TRUSTED_CONSOLE_CATALOG}:
        raise TransactionError("tracked Console domain inventory differs from manifest")

    for name, row in expected_files.items():
        path = TRUSTED_CONSOLE_DATA_PREFIX + name
        content = _revision_bytes(repository, head_commit, path)
        try:
            text = content.decode("utf-8")
            if not text.startswith(TRUSTED_CONSOLE_PART_PREFIX) or not text.endswith(");\n"):
                raise ValueError
            payload = json.loads(
                text[len(TRUSTED_CONSOLE_PART_PREFIX) : -3]
            )
        except (UnicodeError, json.JSONDecodeError, ValueError) as error:
            raise TransactionError(f"trusted Console domain is not assignment-only: {name}") from error
        canonical = (
            TRUSTED_CONSOLE_PART_PREFIX
        ...
        if (
            content != canonical
            or payload.get("domain_generation") != {name: generation_id}
            or row.get("sha256") != "sha256:" + hashlib.sha256(content).hexdigest()
            or row.get("bytes") != len(content)
            or row.get("keys")
            != sorted(key for key in payload if key != "domain_generation")
            or row.get("record_count")
            != _console_payload_count(
                {key: value for key, value in payload.items() if key != "domain_generation"}
            )
            or files.get(name) != expected_index
        ):
            raise TransactionError(f"trusted Console domain binding differs: {name}")

    catalog_bytes = _revision_bytes(repository, head_commit, TRUSTED_CONSOLE_CATALOG)
    try:
        catalog_text = catalog_bytes.decode("utf-8")
        if not catalog_text.startswith(TRUSTED_CONSOLE_CATALOG_PREFIX) or not catalog_text.endswith(";\n"):
            raise ValueError
        catalog = json.loads(
            catalog_text[len(TRUSTED_CONSOLE_CATALOG_PREFIX) : -2]
        )
    except (UnicodeError, json.JSONDecodeError, ValueError) as error:
        raise TransactionError("trusted Console catalog is not assignment-only") from error
    serialized_catalog = "{\n" + ",\n".join(
    ...
    canonical_catalog = (
        TRUSTED_CONSOLE_CATALOG_PREFIX
        + serialized_catalog.replace("</", "<\\/")
        + ";\n"
    ).encode("utf-8")
    if (
        catalog_bytes != canonical_catalog
        ...
        raise TransactionError("trusted Console catalog binding differs")
    return frozenset(expected_paths)


def _console_build_validation_succeeded(run_dir: Path) -> bool:
    path = run_dir / "validation-results.json"
    if not path.is_file() or path.is_symlink():
        return False
    try:
    ...
    for row in rows if isinstance(rows, list) else []:
        ...
        if (
            ...
            and len(command) > 1
            and row.get("id") == "console-build"
            and row.get("returncode") == 0
            and command[1] == "scripts/build_project_console.py"
        ):
            return True
    return False


def validate_trusted_console_pull_request(
    repository: Path,
    *,
    base_commit: str,
    head_commit: str,
    pull_request_author: str,
    pull_request_author_type: str,
    event_actor: str,
) -> dict[str, Any]:
    changed = {
        path
        for _status, path in _changed_paths_against_commit(
            repository,
            base_commit,
            head_commit,
        )
    ...
    console_changed = {
        path
        for path in changed
        if path == TRUSTED_CONSOLE_CATALOG
        or path.startswith(TRUSTED_CONSOLE_DATA_PREFIX)
    }
    if not console_changed:
        return {"applicable": False, "generated_only": False, "paths": []}
    trusted_paths = validate_trusted_console_generation(repository, head_commit)
    if not console_changed.issubset(trusted_paths):
        raise TransactionError("pull request changes an untrusted Console output")
    other_paths = changed - console_changed
    exact_app = (
        pull_request_author == TRUSTED_CONSOLE_PR_ACTOR
        and pull_request_author_type == "Bot"
        and event_actor == TRUSTED_CONSOLE_PR_ACTOR
    )
    if not other_paths:
        if not exact_app:
            raise TransactionError(
                "generated-only Console publication requires the exact Automation App"
            )
    elif not exact_app:
        codeowners_text = _revision_bytes(
            repository,
            head_commit,
            ".github/CODEOWNERS",
        ).decode("utf-8", "strict")

        def owners_for(path: str) -> tuple[str, ...]:
            selected: tuple[str, ...] = ()
            rooted = "/" + path
            for raw in codeowners_text.splitlines():
                ...
                if any(marker in pattern for marker in ("*", "?", "[")):
                    raise TransactionError(
                        "trusted Console check does not accept wildcard CODEOWNERS"
                    )
                matched = (
                    rooted.startswith(pattern)
                    if pattern.endswith("/")
                    else rooted == pattern
                )
                if matched:
                    selected = tuple(fields[1:])
            return selected

        if not any(owners_for(path) for path in other_paths):
            raise TransactionError(
                "ownerless Console output may not mix with ordinary unowned paths"
            )
    return {
        "applicable": True,
        "generated_only": not other_paths,
        "exact_app": exact_app,
        "paths": sorted(console_changed),
    }
...
def classify_publication_range(
    ...
    *,
    ...
    console_candidates = {
        record.path
        for record in records
        if record.path == TRUSTED_CONSOLE_CATALOG
        or record.path.startswith(TRUSTED_CONSOLE_DATA_PREFIX)
    }
    if console_candidates:
        if not _console_build_validation_succeeded(run_dir):
            raise TransactionError(
                "trusted Console publication lacks a successful current-run build"
            )
        trusted_paths = validate_trusted_console_generation(repository, head_commit)
        if not console_candidates.issubset(trusted_paths):
            raise TransactionError("Console publication changed an untrusted output path")
        records = [
            IndexRecord(
                record.path,
                record.mode,
                record.blob,
                record.status,
                "ordinary" if record.path in trusted_paths else record.classification,
            )
            for record in records
        ]
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--pull-request-author-type")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.validate_repository_policy:
        ...
        return 0
    if args.validate_trusted_console_pr:
        required = {
            "base_commit": args.base_commit,
            "head_commit": args.head_commit,
            "pull_request_author": args.pull_request_author,
            "pull_request_author_type": args.pull_request_author_type,
            "event_actor": args.event_actor,
        }
        if any(not isinstance(value, str) or not value for value in required.values()):
            print("trusted Console PR validation requires complete event bindings", file=sys.stderr)
            return 64
        repository = (args.canonical_path or Path.cwd()).resolve()
        validate_trusted_console_pull_request(
            repository,
            base_commit=str(args.base_commit),
            head_commit=str(args.head_commit),
            pull_request_author=str(args.pull_request_author),
            pull_request_author_type=str(args.pull_request_author_type),
            event_actor=str(args.event_actor),
        )
